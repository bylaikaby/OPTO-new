...
%
%  NOTE:
%    Image parameters (e.g. size) may not be correct for "fid".
%
%  EXAMPLE :
%    imgp = pv_imgpar('\\wks8\mridata_wks8\H05.Tm1\55\pdata\1\2dseq');  % RARE
%    imgp = pv_imgpar('\\wks8\mridata_wks8\B07.371\62\pdata\1\2dseq');  % FLASH
%    imgp = pv_imgpar('\\wks8\mridata_wks8\J02.HX1\7\pdata\1\2dseq');   % MDEFT
%    imgp = pv_imgpar('\\wks8\mridata_wks8\B07.371\46\pdata\1\2dseq');  % EPI
%    imgp = pv_imgpar('\\wks21\data\rat.bY2\25\pdata\1\2dseq');         % 3D RARE
%    imgp = pv_imgpar('\\wks21\data\rat.bY2\26\pdata\1\2dseq');         % 3D FLASH
%    imgp = pv_imgpar('\\wks21\data\rat.Oc1\21\pdata\1\2dseq');         % MDEFT
%    imgp = pv_imgpar('\\wks21\data\rat.Oc1\22\pdata\1\2dseq');         % 3D RARE
...
%    imgp = pv_imgpar('\\wks21\data\rat.NW1\18\pdata\1\2dseq');         % 3D EPI
%
%  VERSION :
...
%    1.11 29.06.18 YM  supports 3D-EPI.
%
...
elseif ischar(varargin{1}) && ~isempty(strfind(varargin{1},'ser'))
  ...
  if nargin < 2
    if any(strcmpi({'test'  'test2dseq'}, varargin{1}))
      sub_do_test();
    elseif any(strcmpi({'test2' 'testfid'  }, varargin{1}))
      sub_do_test_fid();
    else
      error(' ERROR %s: missing 2nd arg. as ExpNo.\n',mfilename);
    end
    IMGP = [];  % just to avoid error.
    return;
  end
...
if GET_RECO
  ...
  if length(reco.RECO_size) > 2
    ...
    %if strncmpi(acqp.PULPROG,'<mdeft',6)
      nt = acqp.NI;
    %end
    %if isfield(acqp,'ACQ_dim') && acqp.ACQ_dim == 3,
    if isfield(method,'PVM_SpatDimEnum') && strcmpi(method.PVM_SpatDimEnum,'3D')
      % 3D EPI
      nt = acqp.NR;
    end
  end
  ...
  % !!!! it doesn't work for "fid" of 3D-RARE...
  % FID info
  % acqp.ACQ_dim
  if isfield(acqp,'ACQ_dim_desc') && any(strcmpi(acqp.ACQ_dim_desc,'Spectroscopic'))
    ...
    if isfield(method,'PVM_Matrix')
      ...
      %nx = acqp.ACQ_size(1);
      nx = acqp.ACQ_size(1)/2;  % because of the complex number??
  ...
  switch lower(acqp.GO_block_size)
   case {'standard_kblock_format'}
    nx = ceil(2*nx*NBytes/1024)*1024/NBytes/2;  % x2 for the complex number..
    % to be clarified...
    %nx = nx / 2;  % because of complex number?: for mdeft, not others...
   case {'continuous'}
  end
...
if f_PVM == 1
  if isfield(method,'PVM_VoxArrSize') && any(method.PVM_VoxArrSize)
    ...
    if isfield(method,'PVM_SpatDimEnum') && strcmpi(method.PVM_SpatDimEnum,'3D')
      % 3D EPI
      slitr = acqp.ACQ_repetition_time/1000;
      segtr = segtr*ns;
      imgtr = imgtr*ns;
    end
  end
...
%if isfield(method,'PVM_EpiNShots') && isfield(method,'PVM_SpatDimEnum') && strcmpi(method.PVM_SpatDimEnum,'3D')
if isfield(method,'PVM_SpatDimEnum') && strcmpi(method.PVM_SpatDimEnum,'3D')
IMGP.slithk       = ds;
end
...
fprintf('%s TEST MODE\n',mfilename);
% RARE, etc
IMG{1} = {'RARE';     '\\wks8\mridata_wks8\H05.Tm1\55\pdata\1\2dseq' };
IMG{2} = {'FLASH';    '\\wks8\mridata_wks8\B07.371\62\pdata\1\2dseq' };
IMG{3} = {'MDEFT';    '\\wks8\mridata_wks8\J02.HX1\7\pdata\1\2dseq'  };
IMG{4} = {'EPI';      '\\wks8\mridata_wks8\B07.371\46\pdata\1\2dseq' };
%IMG{5} = {'3D RARE';  '\\wks21\data\rat.bY2\25\pdata\1\2dseq'        };
%IMG{6} = {'3D FLASH'; '\\wks21\data\rat.bY2\26\pdata\1\2dseq'        };
IMG{5} = {'MDEFT';    '\\wks21\data\rat.Oc1\21\pdata\1\2dseq'        };
IMG{6} = {'3D RARE';  '\\wks21\data\rat.Oc1\22\pdata\1\2dseq'        };
IMG{7} = {'ANGIO';    '\\wks8\mridata_wks8\C04.p61\7\pdata\1\2dseq'  };
IMG{8} = {'rpPRESS';  '\\wks21\data\rat.pE2\90\pdata\1\2dseq'        };
IMG{9} = {'3D EPI';   '\\wks21\data\rat.NW1\18\pdata\1\2dseq'        };
...
% RARE, etc
IMG{1} = {'RARE';     '\\wks8\mridata_wks8\H05.Tm1\55\fid' };
IMG{2} = {'FLASH';    '\\wks8\mridata_wks8\B07.371\62\fid' };
IMG{3} = {'MDEFT';    '\\wks8\mridata_wks8\J02.HX1\7\fid'  };
IMG{4} = {'EPI';      '\\wks8\mridata_wks8\B07.371\46\fid' };
%IMG{5} = {'3D RARE';  '\\wks21\data\rat.bY2\25\fid'        };
%IMG{6} = {'3D FLASH'; '\\wks21\data\rat.bY2\26\fid'        };
IMG{5} = {'MDEFT';    '\\wks21\data\rat.Oc1\21\fid'        };
IMG{6} = {'3D RARE';  '\\wks21\data\rat.Oc1\22\fid'        };
IMG{7} = {'ANGIO';    '\\wks8\mridata_wks8\C04.p61\7\fid'  };
IMG{8} = {'rpPRESS';  '\\wks21\data\rat.pE2\90\fid'        };
IMG{9} = {'3D EPI';   '\\wks21\data\rat.NW1\18\fid'        };
