...
%    1.20 31.01.12 YM  new saving style.
%    1.22 05.06.13 YM  save with -v7.3.
%    1.22 14.06.13 YM  bug fix when .dirname is given.
%    1.30 27.03.15 YM  use pv_getpvpars().
%    1.31 03.11.15 YM  supports ASCAN.xx.rawname for PV6.
%
...
for iAna = 1:length(ananames),
  ...
  if sesversion(Ses) >= 2,
    fprintf('%s %s: saving ''%s'' ',...
            datestr(now,'HH:MM:SS'),mfilename,AnaName);
    for N = 1:length(CurScan),
      fprintf('.');
      fname = sigfilename(Ses,N,AnaName);
      eval(sprintf('%s = CurScan{N};',AnaName));
      mmkdir(fileparts(fname));
      save(fname,AnaName,'-v7.3');
    end
  else
    % save anatomy data
...
DataMri = getdirs('DataMri');
if isfield(Ses.sysp,'DataMri'), DataMri = Ses.sysp.DataMri;  end
...
if isfield(ASCAN,'dirname') && ~isempty(ASCAN.dirname),
  ...
  if any(fp),
    DataMri = fp;
    rawname = sprintf('%s%s',fr,fe)
  else
    rawname = ASCAN.dirname;
  end
elseif isfield(ASCAN,'rawname') && ~isempty(ASCAN.rawname)
  [fp,fr,fe] = fileparts(ASCAN.rawname);
  if any(fp),
    DataMri = fp;
    rawname = sprintf('%s%s',fr,fe)
  else
    rawname = ASCAN.rawname;
  end
else
  if isfield(Ses.sysp,'rawname') && any(Ses.sysp.rawname)
    rawname = Ses.sysp.rawname;
  else
    rawname = Ses.sysp.dirname;
  end
end
...
%p = getpvpars(fullfile(DataMri,rawname),ASCAN.scanreco(1),ASCAN.scanreco(2));
p = pv_getpvpars(fullfile(DataMri,rawname,nam));
...
AINFO.name		= AnaName;
AINFO.DataMri   = DataMri;
AINFO.filename  = fullfile(rawname,nam);
