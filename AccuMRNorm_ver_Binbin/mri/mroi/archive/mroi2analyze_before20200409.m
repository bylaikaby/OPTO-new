...
switch lower(MODE_STR)
 ...
 case {'mask'}
  sub_export_mask(ses,grp,RoiName,ROI,UNDO_CROP,EXPORT_AS_NII,NII_COMPATIBLE);
end


% =============================================================
function sub_export_mask(ses,grp,RoiNames,ROI,UNDO_CROP,EXPORT_AS_NII,NII_COMPATIBLE)
% =============================================================
...
if any(UNDO_CROP)
  ...
  if equal(size(roidata),size(voldata))
    % to avoid error...
    ix = 1:size(voldata,1);
    iy = 1:size(voldata,2);
  else
...
if ~isempty(hdr)
  ...
  sub_write_infotxt(txtfile,hdr,UNDO_CROP,EXPORT_AS_NII,NII_COMPATIBLE);
end

return

% =============================================================
function sub_write_infotxt(TXTFILE,HDR,UNDO_CROP,EXPORT_AS_NII,NII_COMPATIBLE)
% =============================================================
