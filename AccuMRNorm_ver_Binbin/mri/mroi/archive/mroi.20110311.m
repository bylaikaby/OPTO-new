...
hMain = figure(...
'Name',...
'MROI: Graphics Interface for Region-of-Interest (ROI) selection (v1.34.20101130)',...
	'NumberTitle','off', ...
...
% COMBO : ROI action
ActCmd = {'No Action','Coordinate','Append','Replace','Electrodes',...
...
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+22 H 6 1.25],...
    'String','Slice: ','FontWeight','bold',...
    'HorizontalAlignment','left','fontsize',9,...
    'BackgroundColor',BKGCOL);
AtlasSliceBarSldr = uicontrol(...
    'Parent',hMain,'Style','slider',...
    'Units','char','Position',[XDSP+28 H 13 1.5],...
...
AtlasSlice = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+43 H 4 1.5],...
...
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+52 H 30 1.25],...
    'String','AtlasCrop (mm): ','FontWeight','bold',...
    'HorizontalAlignment','left','fontsize',9,...
    'BackgroundColor',BKGCOL);
AtlasCropEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+72, H 22 1.5],...
...
crophelp=uicontrol(...
    'Parent',hMain,'Style','pushbutton',...
    'Units','char','Position',[XDSP+95, H 5 1.5],...
...
AtlasRotEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+IMG2XOFS+45, H 8 1.5],...
    'Callback','mroi(''Main_Callback'',gcbo,''imgdraw'',[])',...
...
OverlayCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[figW-15 H 12 1.5],...
   'Callback','mroi(''Main_Callback'',gcbo,''imgdraw'',[])',...
    ...
    if isfield(anap.mroi,'atlas') && ~isempty(anap.mroi.atlas),
          ...
          if isfield(anap.mroi.atlas,'imgcrop')
              if iscell(anap.mroi.atlas.imgcrop)
                  ...
                  set(wgts.AtlasCropEdt,'String',num2str(imgcropmat(1,:)))
                  setappdata(hMain,'atlimgcrop',imgcropmat)
              else
                set(wgts.AtlasCropEdt,'String',num2str(anap.mroi.atlas.imgcrop))
                setappdata(hMain,'atlimgcrop',num2str(anap.mroi.atlas.imgcrop))
              end
          else
                set(wgts.AtlasCropEdt,'String',num2str([1 1 1 1]))
                setappdata(hMain,'atlimgcrop',num2str([1 1 1 1]))
          end
...
%           end
           ...
           try %try to load transformed atlas
                Atlastform=load('atlas_tform');
                setappdata(hMain,'Atlas',Atlas);
                set(wgts.AtlasCmb,'String',{'Hor DV','Hor VD','Cor AP','Cor PA','Atlas_tform'});
                set(wgts.AtlasCmb,'Value',5);
                Atlas.Roidef_atlas=Atlastform.Roidef_atlas;
                clear Atlastform;
           catch
                %no catch needed
           end
...
if ~isempty(atlslice)
 ...
 % Atlas.scale=get(wgts.AtlasScaleEdt,'String');% gives the scale for the image [sx sy]
  ...
  OverlayCheck=get(wgts.OverlayCheck,'Value');
        if isequal(str2num(Atlas.crop),[1 1 1 1]),
            Atlas.crop=num2str([1 1 size(tmpanaimg,1)*Atlas.ds(1) size(tmpanaimg,2)*Atlas.ds(2)]);
            set(wgts.AtlasCropEdt,'String',Atlas.crop);
        end
  if OverlayCheck==1
 ...
 case {'load'}
  ...
  if exist(RoiFile,'file') && ~isempty(who('-file',RoiFile,RoiVar)),
    ...
    Roi.ele = subUpdateEle(wgts,ses,RoiVar);
...
for N = 1:length(grpnames),
  ...
  if isfield(tmpgrp,'ele') && isfield(tmpgrp.ele,'mcoords') && ~isempty(tmpgrp.ele.mcoords),
    ...
    RoiEle = {};
    for N = 1:length(tmpgrp.ele.mcoords),
      RoiEle{N}.ele = N;
...
if any(strcmpi(RoiDraw,{'all','all ROIs', 'current'})),
  ...
  for N = 1:length(RoiRoi),
    ...
    if isempty(px) || isempty(py),
      [px,py] = find(imresize(double(RoiRoi{N}.mask),IMGDIMS.ana,'nearest'));
