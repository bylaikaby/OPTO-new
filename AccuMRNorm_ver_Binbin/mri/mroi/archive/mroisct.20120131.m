...
if nargin < 3,
  ...
  if exist('tcImg.mat','file') & ~isempty(who('-file','tcImg.mat',grp.name)),
    tmp = load('tcImg.mat',grp.name);
    eval(sprintf('tcImg = tmp.%s;',grp.name));
    fprintf(' mroisct: tcImg loaded from "tcImg.mat".\n');
  else
    ExpNo = grp.exps(1);
    fname = catfilename(ses,ExpNo,'tcimg');
    tcImg = matsigload(fname);
    tcImg.dat = mean(tcImg.dat,4);
    fprintf(' mroisct: tcImg loaded from "%s".\n',fname);
  end
end
...
if ~isempty(grp.ana) & (size(tcImg.dat,3) ~= length(grp.ana{3})),
