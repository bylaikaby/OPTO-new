...
if isempty(cormap)
  if ~isempty(atlas)%add only atlas-overlay exists
    ...
    if fuse_atlas% fusion version
      ...
      imgsz = size(img);
      img   = reshape(img,  [imgsz(1)*imgsz(2) imgsz(3)]);
      atlas = reshape(atlas,[imgsz(1)*imgsz(2) imgsz(3)]);
      % ignore white-region
      tmpidx = atlas(:,1) > 0.9 & atlas(:,2) > 0.9 & atlas(:,3) > 0.9;
      tmpidx = find(~tmpidx);
      img(tmpidx,:) = atlas(tmpidx,:);
      img   = reshape(img,  imgsz);
      atlas = reshape(atlas,imgsz);
  ...
  % index to rgb
  img(isnan(img(:)) == 1) = 1; % to avoid error in ind2rgb
...
set(gca,'xlim',[0.5 size(img,1)+0.5],'ylim',[0.5 size(img,2)+0.5]);
...
if ~isempty(actidx)
  ...
  cormap(isnan(cormap(:)) == 1) = 1; % to avoid error in ind2rgb
