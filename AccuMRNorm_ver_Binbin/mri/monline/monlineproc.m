...
%    1.02 06.07.18 YM  supports 3D-EPI.
%    1.03 20.02.19 YM  supports apply_hrf as fhemo|hemo|Cohen.
%    1.04 21.02.19 YM  clean-up.
%    1.05 06.03.19 YM  skips tripilot when any error.
%
...
if isempty(iSTIM)
  iSTIM.stmtypes = {'blank','stim', 'blank'};
  iSTIM.tvol     = [2 6 14];
  iSTIM.rep      = 20;
end
...
if isnumeric(ANAP.apply_hrf)
  if any(ANAP.apply_hrf)
    % ANAP.apply_hrf = 1;
    ANAP.apply_hrf = 'Cohen';
  else
    % ANAP.apply_hrf = 0;
    ANAP.apply_hrf = 'none';
  end
...
hrfopt = {'gampdf' 'hemo' 'fgampdf' 'fhemo' 'vfgampdf','vfhemo' 'cohen' 'spm'};
if any(strcmpi(hrfopt,ANAP.apply_hrf))
  fprintf(' model-hrf(%s).',ANAP.apply_hrf);
  STIM = subConvolveHRF(STIM,pvpar.imgtr,ANAP.apply_hrf);
end
fprintf('\n  ');
...
if length(AACQP.ACQ_slice_offset) > 1
  ...
  if length(ARECO.RECO_fov) >= 3
    SIG.anads(3) = ARECO.RECO_fov(3)*10/size(IMGDAT,3);
  end
end
...
if length(ACQP.ACQ_slice_offset) > 1
  ...
  if length(RECO.RECO_fov) >= 3
    pvpar.ds(3) = RECO.RECO_fov(3)*10/pvpar.nsli;
  end
end
% timings in seconds
if ~isempty(METHOD)
  ...
  if isfield(METHOD,'PVM_SpatDimEnum') && strcmpi(METHOD.PVM_SpatDimEnum,'3D')
    % 3D EPI
    pvpar.slitr = ACQP.ACQ_repetition_time/1000;
    pvpar.segtr = pvpar.segtr*pvpar.nsli;
    pvpar.imgtr = pvpar.imgtr*pvpar.nsli;
  end
else
...
ds(3) = ds(1);
  ...
  TRIPILOT = [];
  fprintf('\n ERROR %s.subGetTripilot: not tripilot (%s), skipping tripilot...\n',mfilename,TRIFILE);
  return
end
...
% fprintf(' global-.');
% IMGDAT = subSubtractGlobal(IMGDAT);
...
function IMGDAT = subSubtractGlobal(IMGDAT)
