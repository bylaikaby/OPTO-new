...
%    0.93 17.07.18 YM  fixed problems of graphic handles (2014b).
%
...
anaminv = 0;
%anamaxv = round(nanmean(ONLINE.tripilot.dat(:))*10.0);
anamaxv = round(nanstd(double(ONLINE.tripilot.dat(:))))*10;
anamaxv = round(anamaxv/100)*100;
%max(ONLINE.tripilot.dat(:))
anagamma = 1.5;
...
if iscell(ONLINE.imgfile),
  ONLINE.imgfile = ONLINE.imgfile{1};
  ONLINE.scanreco = ONLINE.scanreco(1,:);
end
...
switch lower(eventdata),
 ...
 case {'redraw'}
  ...
  if size(IMGDAT,3) > 3
    if isfield(ONLINE.tripilot,'pvpar') && isfield(ONLINE.tripilot.pvpar,'acqp') && isfield(ONLINE.tripilot.pvpar.acqp,'ACQ_slice_offset'),
      TRI_IDX = find(ONLINE.tripilot.pvpar.acqp.ACQ_slice_offset == 0);
    else
      TRI_IDX = [2 5 8];
    end
  else
  ...
  image(tmpX,tmpY,squeeze(IMGDAT(:,:,TRI_IDX(1),:)));
  set(gca,'color',[0 0 0],'xlim',XLIM,'ylim',YLIM);
  daspect([2 2 1]);
  set(gca,'Tag','TraAxs');
  xlabel('X (mm)');  ylabel('Y (mm)');
  text(0.01,0.99,'XY','color',[0.9 0.9 0],...
       'units','normalized','verticalalignment','top');
  
  axes(wgts.SagAxs); cla;
  image(tmpX,tmpY,squeeze(IMGDAT(:,:,TRI_IDX(2),:)));
  set(gca,'color',[0 0 0],'xlim',XLIM,'ylim',YLIM);
  daspect([2 2 1]);
  set(gca,'Tag','SagAxs');
  xlabel('Y (mm)');  ylabel('Z (mm)');
  text(0.01,0.99,'YZ','color',[0.9 0.9 0],...
       'units','normalized','verticalalignment','top');

  axes(wgts.CorAxs); cla;
  image(tmpX,tmpY,squeeze(IMGDAT(:,:,TRI_IDX(3),:)));
  set(gca,'color',[0 0 0],'xlim',XLIM,'ylim',YLIM);
  daspect([2 2 1]);
  set(gca,'Tag','CorAxs');
  xlabel('X (mm)');  ylabel('Z (mm)');
  text(0.01,0.99,'XZ','color',[0.9 0.9 0],...
       'units','normalized','verticalalignment','top');
