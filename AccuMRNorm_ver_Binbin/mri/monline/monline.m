...
%    1.03 21.02.19 YM  try recovering parameters from the base-workspace.
%
...
% try getting parameters from the existing result in the base-workspace.
DEF = subGetDefaultFromWorkspace(DEF);
...
uicontrol(...
    ...
    'HorizontalAlignment','left',...
    'TooltipString','Apply selected HRF to the boxcar model',...
    'FontWeight','Bold','FontName','FixedWidth');
...
% ==================================================================
function DEF = subGetDefaultFromWorkspace(DEF)
% ==================================================================
if ~any(evalin('base','exist(''SIG'',''var'')')),  return;  end
SIG = evalin('base','SIG');
...
if isfield(SIG,'stm') && isfield(SIG.stm,'stmtypes')
  tmpstr = '';
  for N = 1:length(SIG.stm.stmtypes)
    tmpstr = [tmpstr SIG.stm.stmtypes{N} ' '];
  end
  DEF.stmtypes = deblank(tmpstr);
  DEF.stimtvol = deblank(sprintf('%d ',SIG.stm.dtvol{1}));
  if isfield(SIG,'pvpar') && isfield(SIG.pvpar,'nt')
    DEF.stimrep  = round(SIG.pvpar.nt/sum(SIG.stm.dtvol{1}));
  end
end

if isfield(SIG,'anap')
  f = {'imgnormalize' 'xysmooth' 'centroid' 'detrend' 'xform' 'average' ...
       'apply_hrf' 'corrana' 'glmana' 'ttest' };
  for N = 1:length(f)
    if isfield(SIG.anap,f{N})
      DEF.(f{N}) = SIG.anap.(f{N});
    end
  end
end
