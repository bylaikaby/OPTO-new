...
%
%  See also MONLINE MONLINEPROC MONLINEPVPAR
...
InfoTxt = uicontrol(...
    ...
    'Units','char','Position',[XDSP+12+XSZ+12 H 30 9],...
...
if isempty(ROI),
  switch lower(SelectValue),
   case {'pos','positive','neg','negative'}
    ...
    tmptxt{1} = sprintf('%s(P<%s,%s,Nvox=%d)',StatName,get(wgts.AlphaEdt,'string'),...
                        SelectValue,length(tmpidx{1}));
   otherwise
    % plot both pos and neg
    tmpidx{1} = find(STATMAP.mask.dat(:) > 0 & STATMAP.dat(:) > 0);
    tmptxt{1} = sprintf('%s(P<%s,pos,Nvox=%d)',StatName,get(wgts.AlphaEdt,'string'),...
                        length(tmpidx{1}));
    tmpidx{2} = find(STATMAP.mask.dat(:) > 0 & STATMAP.dat(:) < 0);
    tmptxt{2} = sprintf('%s(P<%s,neg,Nvox=%d)',StatName,get(wgts.AlphaEdt,'string'),...
                        length(tmpidx{2}));
  end
else
  tmpidx{1} = [];
...
if get(wgts.TCHoldCheck,'Value') == 0,
  ...
  if isempty(ROI),
    if length(tmpidx) == 1,
      tmpstr = sprintf('Nvox=%d P<%s',...
                       size(tcdat,2),get(wgts.AlphaEdt,'String'));
    else
      tmpstr = sprintf('Nvox=%d/%d P<%s',...
                       length(tmpidx{1}),length(tmpidx{2}),get(wgts.AlphaEdt,'String'));
    end
  else
...
tmpstr = sprintf('%s %s(%s,P<%s,cluster=%d)',...
                 tmpstr,StatName,SelectValue,...
                 get(wgts.AlphaEdt,'String'),get(wgts.ClusterCheck,'value'));
