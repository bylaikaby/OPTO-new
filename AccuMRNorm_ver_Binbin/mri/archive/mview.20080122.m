...
roinames = { 'ALL', roinames{:}, 'PickArea' };
...
clear roinames idx;
% Select Area Button %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SelectAreaBtn = uicontrol(...
    'Parent',hMain,'Style','PushButton',...
    'Units','char','Position',[XDSP+40 H 14 1.5],...
    'String','Pick Area','Tag','SelectAreaBtn',...
    'Callback','mview(''Main_Callback'',gcbo,''select-area'',guidata(gcbo))',...
    'TooltipString','Select a ROI (only works in PickArea mode)',...
    'FontWeight','bold');
...
DrawEleCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+70 H 20 1.5],...
    'Callback','mview(''Main_Callback'',gcbo,''redraw-image'',guidata(gcbo))',...
...
OverlayCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+79 H 20 1.5],...
...
ZReverseCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+110 H 20 1.5],...
...
SaveMaskBtn = uicontrol(...
    'Parent',hMain,'Style','PushButton',...
    'Units','char','Position',[XDSP+127 H-2 42 1.5],...
...
SelectCorrCmb = uicontrol(...
    'Parent',hMain,'Style','Popupmenu',...
    'Units','char','Position',[XDSP+10+XSZ+15 H+YSZ-7.5 27 1.5],...
    'Callback','mview(''Main_Callback'',gcbo,''select-stat'',guidata(gcbo))',...
    'String',{'pos corr','neg corr','pos+neg'},...
    'Tag','SelectCorrCmb','Value',1,...
    'TooltipString','Select corr to plot',...
    'FontWeight','bold');
...
switch lower(eventdata),
 ...
 case {'init-statmap'}
  ...
  % if ROI is PickArea
  if strcmpi(RoiName,'PickArea'),
    %LightboxView_Callback(hObject,'init',[]);
    STATMAP = getappdata(wgts.main,'STATMAP');
    if isempty(STATMAP),
      STATMAP = subGetStatNull(ROITS,wgts,alpha,'ALL');
 ...
 case {'select-stat'}
  RoiName = get(wgts.RoiCmb,'String');  RoiName = RoiName{get(wgts.RoiCmb,'Value')};
  if strcmpi(RoiName,'PickArea'),  return;  end
 ...
 case {'select-area'}
  ...
  RoiName = get(wgts.RoiCmb,'String');  RoiName = RoiName{get(wgts.RoiCmb,'Value')};
  if ~strcmpi(ViewMode,'orthogonal') & strcmpi(RoiName,'PickArea'),
    %LightboxView_Callback(hObject,'redraw',[]);
    hPickROI = findobj(wgts.LightboxAxs,'tag','PickROI');
    delete(hPickROI);
    subPickArea(wgts);
    Main_Callback(hObject,'redraw-timecourse',[]);
  end
...
[NRow NCol] = subGetNRowNCol(size(ROITS{1}{1}.ana),ROITS{1}{1}.ds,iDimension,wgts);
NROWCOL.NRow = NRow;
NROWCOL.NCol = NCol;
...
for T = 1:length(ROITS{1}),
  ...
  tmpmap.mask.pick    = zeros(EPIDIM,'int8');
...
SelectCorr = get(wgts.SelectCorrCmb,'String');
SelectCorr = SelectCorr{get(wgts.SelectCorrCmb,'Value')};
...
for iTrial = 1:length(TrialNo),
  ...
  if strcmpi(RoiName,'PickArea'),
    idx = find(STATMAP{T}.mask.pick(:) > 0);
  else
    idx = find(STATMAP{T}.mask.dat(:) > 0);
  end
  %'size(idx)'
  ...
  if isempty(found),
    ...
    %% 12 AUG 2007 NIKOS - Small temporary change to see better the curves
    %% YUSUKE: Please have a switch for the error bars, and include the confidence interval
    %% plotting I put in SHOWMAP
    %% OLD: hDATA(end+1) =
    %% errorbar(tmpt,tmpm,tmps,'color',tmpcol,'tag','tcdat','UserData',tmptxt);
    %% NEW:
    hDATA(end+1) = plot(tmpt,tmpm,'color',tmpcol,'tag','tcdat','UserData',tmptxt);
  end
  hold on;
end
...
function [NRow NCol] = subGetNRowNCol(IMGDIM,PIXDIM,iDimension,wgts)
...
while 1,
  ...
  if length(xi) > 2,
    ...
    BW = poly2mask(mod(xi,EPIDIM(1)),mod(yi,EPIDIM(2)),EPIDIM(2), EPIDIM(1));
  
    % calculate the slice number in which the ROI was selected
    SliceNo = (((NROWCOL.NRow - floor(yi(1)/EPIDIM(2)) - 1) * NROWCOL.NCol) + (floor(xi(1)/EPIDIM(1)) + 1)) + ((ipage - 1) * (NROWCOL.NRow * NROWCOL.NCol));
  
    BW = logical(BW');
    BW = flipdim(BW,2);   % flip Y
    %BW = logical(imresize(BW,EPIDIM(1:2)));
    
    % now register the new roi
    hold on;
    plot(xi,yi,'color',[0 1 0],'tag','PickROI');
    PICKMASK(:,:,SliceNo) = BW | PICKMASK(:,:,SliceNo);

    % just for debug
    if 0,
      figure;
      subplot(2,1,1); imagesc(BW');
      hold on;
      plot(mod(xi,EPIDIM(1)),EPIDIM(2)-mod(yi,EPIDIM(2)),'color',[1 0 0],'tag','PickROI');      
      subplot(2,1,2); imagesc(squeeze(STATMAP{1}.mask.dat(:,:,SliceNo))');
      hold on;
      plot(mod(xi,EPIDIM(1)),EPIDIM(2)-mod(yi,EPIDIM(2)),'color',[1 0 0],'tag','PickROI');      
    end
...
%STATMAP{1}.mask.dat = tmpmap.mask.dat;
for iTrial = 1:length(TrialNo),
  T = TrialNo(iTrial);
  STATMAP{T}.mask.pick = PICKMASK;
end
setappdata(wgts.main,'STATMAP',STATMAP);
