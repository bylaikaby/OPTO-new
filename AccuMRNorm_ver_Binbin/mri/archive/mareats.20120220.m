function [roiTs IsChanged] = mareats(SESSION,ExpNo,SigName,ARGS)
...
% NKL   12.03.04
% Chand 03 Nov 2005
% YM    16.04.07  if no ROI, then select all voxels for quick analysis.
% YM    15.11.07  ITOSDU can be a cell array of {'method','epoch'}
% YM    25.11.10  computes SNR as .snr
% YM    30.03.11  supports IROIFILTER
% YM    17.01.12  supports IFILTER3D, SigName.
% YM    29.01.12  .ana/.snr as 'single' for less memory.
...
if exist('GrpName','var'),
  ...
  if USE_REALIGNED == 0 && exist(catfilename(Ses,ExpNo,'tcImg.bak'),'file'),
    fprintf('[tcImg.bak]');
...
% LOAD Roi
if any(strcmpi(whofile('roi.mat'),grp.grproi)),
%if exist('roi.mat','file'),
  Roi = matsigload('roi.mat',grp.grproi); % Load the ROI of that group
else
  %fprintf('mareats: Roi.mat does not exist; Run mroi\n');
  %return;
  fprintf('mareats: ''%s'' not in Roi.mat, all voxels are included.\n',grp.grproi);
...
if IRESAMPLE,
  ...
  for N=1:length(roiTs),
    roiTs{N} = sigresample(roiTs{N},IRESAMPLE);
    rts.dx = roiTs{N}.dx;
  end;
end;
...
% NOTE THAT 'DATOFFS' is computed for normalization below.
if IRESAMPLE,
  fprintf(' Resampling with DX=%g.',IRESAMPLE);
  for AreaNo = 1:length(roiTs),
    roiTs{AreaNo} = sigresample(roiTs{AreaNo},IRESAMPLE);
    roiTs{AreaNo}.stm.voldt = IRESAMPLE;
  end;
end;
...
switch lower(method),
 ...
 otherwise
  ...
  if any(strcmpi(method, {'percent','frac'})) && DC_removed > 0,
