...
hMain = figure(...
'Name',...
'MROI: Graphics Interface for Region-of-Interest (ROI) selection (v1.33.20101117)',...
	'NumberTitle','off', ...
...
ActCmd = {'No Action','Append','Replace','Electrodes',...
          'Midline','Ant.Commisure','LR-Separate',...
          'Reset CURSOR',...
...
RoiDrawCmb = uicontrol(...
    ...
    'String',{'all','all ROIs', 'current','markers','none'},'FontWeight','bold',...
...
uicontrol(...
    ...
    'HorizontalAlignment','left','fontsize',9,...
    'BackgroundColor',BKGCOL);
imgpro = {'Mean-Img','Median-Img','Max-Img','Min-Img','Std-Img','StmStd-Img','Cv-Img','T0-Img','Tend-Img'};
...
AtlasCmb = uicontrol(...
    'Parent',hMain,'Style','popupmenu',...
    'Units','char','Position',[XDSP+9 H 10 1.5],...
...
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+24 H 10 1.25],...
    'String','Slice: ','FontWeight','bold',...
    'HorizontalAlignment','left','fontsize',9,...
    'BackgroundColor',BKGCOL);
AtlasSlice = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+40 H 4 1.5],...
...
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+48 H 12 1.25],...
    'String','AtlasCrop: ','FontWeight','bold',...
    'HorizontalAlignment','left','fontsize',9,...
    'BackgroundColor',BKGCOL);
AtlasCropEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+62, H 22 1.5],...
    'Callback','mroi(''Main_Callback'',gcbo,''imgdraw'',[])',...
...
crophelp=uicontrol(...
    'Parent',hMain,'Style','pushbutton',...
    'Units','char','Position',[XDSP+84, H 5 1.5],...
...
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+IMG2XOFS+3 H 12 1.25],...
    'String','AtlasScale:','FontWeight','bold',...
    'HorizontalAlignment','left','fontsize',9,...
    'BackgroundColor',BKGCOL);
AtlasScaleEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+IMG2XOFS+17, H 10 1.5],...
    'Callback','mroi(''Main_Callback'',gcbo,''imgdraw'',[])',...
    'String','inactive','Tag','AtlasScaleEdt',...
    'HorizontalAlignment','left',...
    'TooltipString','set scaling for Atlas',...
    'FontWeight','bold','BackgroundColor','white');

uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+IMG2XOFS+30 H 15 1.25],...
    'String','AtlasRotation:','FontWeight','bold',...
    'HorizontalAlignment','left','fontsize',9,...
    'BackgroundColor',BKGCOL);
AtlasRotEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+IMG2XOFS+45, H 4 1.5],...
    'Callback','mroi(''Main_Callback'',gcbo,''imgdraw'',[])',...
...
MirrorCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[figW-30 H 20 1.5],...
   'Callback','mroi(''Main_Callback'',gcbo,''imgdraw'',[])',...
    'Tag','MirrorCheck','Value',0,...
    'String','Mirror image','FontWeight','bold',...
    'TooltipString','Mirror Atlas overlay','BackgroundColor',get(hMain,'Color'));
OverlayCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[figW-13 H 12 1.5],...
   'Callback','mroi(''Main_Callback'',gcbo,''imgdraw'',[])',...
...
if isfield(anap,'mroi')
    ...
    %will get defaults
    if isfield(anap.mroi,'atlas')
          if isfield(anap.mroi.atlas,'set')
              if strcmpi(anap.mroi.atlas.set,'hor')
          ...
          if isfield(anap.mroi.atlas,'slice')
              ...
              set(wgts.AtlasSlice,'String',num2str(109))
              setappdata(hMain,'atlslice',109)
          end
          if isfield(anap.mroi.atlas,'imgcrop')
                set(wgts.AtlasCropEdt,'String',num2str(anap.mroi.atlas.imgcrop))
          else
                set(wgts.AtlasCropEdt,'String',num2str([1 1 50 50]))
          end
          if isfield(anap.mroi.atlas,'imgscale')
                set(wgts.AtlasScaleEdt,'String',num2str(anap.mroi.atlas.imgscale))
          else
                set(wgts.AtlasScaleEdt,'String',num2str([30 30]))
          end
           ...
           if isfield(anap.mroi.atlas,'mirror')
                 set(wgts.MirrorCheck,'Value',anap.mroi.atlas.mirror)
           else
               set(wgts.MirrorCheck,'Value',0)
           end
           ...
           atlasfile=fullfile(fileparts(mfilename('fullpath')),'mroiatlas','atlas.mat'); %locate atlas
           Atlas=load(atlasfile); %load atlas
           setappdata(hMain,'Atlas',Atlas);
    else
        set(wgts.OverlayCheck,'Enable','off');
        set(wgts.AtlasScaleEdt,'Enable','off');
...
switch lower(eventdata),
 ...
 case {'edit-atlcrop'}
  ...
  atlas.slice=get(wgts.AtlasSlice,'String');% gives the image
  ...
  atlas.mirror=get(wgts.MirrorCheck,'Value');
  atlas.rotate=get(wgts.AtlasRotate,'String');
  [atlimg mapatlas]=getatlasimg(atlas);%load and scale image
  crop=crophelp(atlimg,mapatlas);
  set(wgts.AtlasCropEdt,'String',int2str(crop));
 ...
 case {'edit-atlslice'}
  ...
  if atlas.atlas==1  
    atlas.slice=checklimits(atlas.slice,105,197,1,105);
    set(wgts.AtlasSlice,'String',num2str(atlas.slice));
  else
    atlas.slice=checklimits(atlas.slice,33,183,1,183);
    set(wgts.AtlasSlice,'String',num2str(atlas.slice));
  end
  setappdata(wgts.main,'atlslice',str2num(get(wgts.AtlasSlice,'String'))+1-round(get(wgts.SliceBarSldr,'Value')))
  mroi('Main_Callback',wgts.main,'imgdraw');
 ...
 case {'slice-slider'}		% HERE WE DISPLAY THE IMAGES
  ...
  if get(wgts.OverlayCheck,'Value')
      atlas.atlas=get(wgts.AtlasCmb,'Value'); %gives the folder
      atlas.slice=str2num(get(wgts.AtlasSlice,'String'));
      atlsliceold=getappdata(wgts.main,'atlslice');
      if length(atlsliceold)>1
         atlslice=atlsliceold(SLICE);
      else
          %check for limits of the atlas and correct if needed
          if atlas.atlas==1 
              if atlas.slice<105
                    setappdata(wgts.main,'atlslice',105+(SLICE-1)*2)
              elseif atlas.slice>197
                    setappdata(wgts.main,'atlslice',197-(SLICE-1)*2)
              end
              atlsliceold=checklimits(atlsliceold,105,193,1);
              atlslice=atlsliceold+(SLICE-1)*2;
          else   
            if atlas.slice<35
                    setappdata(wgts.main,'atlslice',33+(SLICE-1)*2)
             elseif atlas.slice>183
                    setappdata(wgts.main,'atlslice',183-(SLICE-1)*2)
            end
            atlsliceold=checklimits(atlsliceold,37,183,1);
            atlslice=atlsliceold-(SLICE-1)*2;
          end
      end     
      set(wgts.AtlasSlice,'String',num2str(atlslice));
  end
  mroi('Main_Callback',wgts.main,'imgdraw');
 ...
 case {'imgdraw'}
  ...
  Atlas.mirror=get(wgts.MirrorCheck,'Value');
  Atlas.rotate=get(wgts.AtlasRotate,'String');
  OverlayCheck=get(wgts.OverlayCheck,'Value');
  if OverlayCheck==1
  ...
  % DRAW ROIS
  axes(wgts.ImageAxs); 
  subDrawAnaROIs(wgts,RoiRoi,RoiEle,ses,SLICE,COLORS,RoiML,RoiAC,RoiLR);
  ...
  % DRAW ROIs
  subDrawEpiROIs(wgts,RoiRoi,RoiEle,ses,SLICE,COLORS,IMGDIMS,RoiML,RoiAC,RoiLR);
...
switch lower(COMMANDSTR),
 case {'append'}
  ...
  while 1,
    ...
    %x = px(1) - 2;  y = py(1) - 2;
    text(x,y,strrep(roiname,'_','\_'),'color',COLORS(cidx),'fontsize',8);
  end
  setappdata(wgts.main,'RoiRoi',RoiRoi);
...
function subDrawAnaROIs(wgts,RoiRoi,RoiEle,ses,SLICE,COLORS,RoiML,RoiAC,RoiLR)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if any(strcmpi(RoiDraw,{'all','all ROIs', 'current'})),
  ...
  for N = 1:length(RoiRoi),
    ...
    if ~isempty(CurRoiName) && ~any(strcmpi(RoiRoi{N}.name,CurRoiName)),  continue;  end
    ...
    % x = px(1) - 2;  y = py(1) - 2;
    text(x,y,strrep(roiname,'_','\_'),'color',COLORS(cidx),'fontsize',10,'tag','roi','fontweight','bold');
  end
end

if any(strcmpi(RoiDraw,{'all','current','markers'})),
  % draw electrodes
  for N = 1:length(RoiEle)
...
function subDrawEpiROIs(wgts,RoiRoi,RoiEle,ses,SLICE,COLORS,IMGDIMS,RoiML,RoiAC,RoiLR)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if any(strcmpi(RoiDraw,{'all','all ROIs', 'current'})),
  ...
  for N = 1:length(RoiRoi),
    ...
    if isempty(RoiRoi{N}.px) || isempty(RoiRoi{N}.py),
      plot(px,py,'color',COLORS(cidx),'linestyle','none',...
           'marker','s','markersize',3,'tag','roi','markerfacecolor',COLORS(cidx));
    else
      plot(px,py,'color',COLORS(cidx),'tag','roi');
    end;
    x = min(px) - 4;  y = min(py) - 2; if x<0, x=1; end;
