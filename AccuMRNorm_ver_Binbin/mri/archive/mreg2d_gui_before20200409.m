...
%
%  VERSION :
%    0.90 28.02.12 YM@MPI  pre-release
%    0.91 17.10.12 YM@MPI  "York" works when .ds differs.
%    0.92 06.12.13 YM@MPI  supports "move to the current SRC".
%    0.93 09.12.13 YM@MPI  supports more action commands (copy/paste etc).
%    0.94 29.05.14 YM@MPI  supports more transformation types.
%    0.95 22.04.16 YM@MPI  fixed problem on graphics handles (2014b).
%    0.96 12.09.19 YM@MPI  use fitgeotrans/imwarp (incompatibility in 2019a).
%
%  See also mana2epi cp2tform imtransform anz_read fitgeotrans imwarp
...
uicontrol(...
    ...
    'String',{'No Action' 'Append' 'Replace END' 'Replace ALL' 'Remove END' 'Remove X' 'Remove ALL' 'Copy' 'Paste to the current REF/SRC' 'Move to the current SRC' 'CLEAR ALL SLICES'},...
    'Value',1,...
...
uicontrol(...
    ...
    'Tag','PointCursorCmb','Value',3,'FontWeight','Bold');
uicontrol(...
    'Parent',hMain,'Style','Text',...
...
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[IMGXOFS+87 H-2.3 15 1.5],...
    'String','Trans. Type: ','FontWeight','bold',...
    'HorizontalAlignment','right','fontsize',9,...
    'BackgroundColor',BKGCOL);
uicontrol(...
    'Parent',hMain,'Style','popupmenu',...
    'Units','char','Position',[IMGXOFS+103 H-2 30 1.5],...
    'String',{'none' 'nonreflective similarity','similarity','linear conformal','affine','projective','polynomial','piecewise linear','lwm'},...
    'Value',9,...
    'TooltipString','Transformation Type, see/help cp2tform/fitgeotrans',...
...
switch lower(eventdata)
 ...
 case {'save-src'}
  ...
  if get(wgts.ForceTransformCheck,'Value') > 0
    ...
    for N = 1:size(REF.dat,3)
      ...
      if any(strcmpi(tmpmethod,'none'))
      elseif length(T_IMAGE.points) < N || isempty(T_IMAGE.points(N).refx)
        tmptxt = sprintf('SLICE(%d) ERROR : CAN''T TRANSFORM, NO FIDUCIAL POINTS.',N);
  ...
  % 2019.09.12: don't keep .regimg(N).tform.tform, it may cause a problem between different
  % matlab versions when loading.
  if isfield(T_IMAGE,'regimg')
    for N = 1:numel(T_IMAGE.regimg)
      if isfield(T_IMAGE.regimg(N).tform,'tform')
        T_IMAGE.regimg(N).tform = rmfield(T_IMAGE.regimg(N).tform,'tform');
      end
    end
  end
 ...
 case {'ref-slice', 'ref-slider'}
  ...
  elseif get(wgts.YorkCheck,'Value') > 0
    SRC = getappdata(wgts.main,'SRC');
    if size(REF.dat,3) ~= size(SRC.dat,3)
      if ~isempty(T_IMAGE.points) && any(T_IMAGE.points(1).slice) 
        iSlice2 = round((iSlice-1)*REF.ds(3)/SRC.ds(3)) + T_IMAGE.points(1).slice;
        %iSlice2 = floor((iSlice-1)*REF.ds(3)/SRC.ds(3)) + T_IMAGE.points(1).slice;
      else
        iSlice2 = round((iSlice-1)*REF.ds(3)/SRC.ds(3)) + 1;
        %iSlice2 = floor((iSlice-1)*REF.ds(3)/SRC.ds(3)) + 1;
      end
    else
      iSlice2 = iSlice;
    end
    if iSlice2 > 0 && iSlice2 <= size(SRC.dat,3)
      set(wgts.SrcSliceEdt,'String',sprintf('%d',iSlice2));
      set(wgts.SrcSldr,'Value',iSlice2);
      Main_Callback(wgts.main,'redraw-src');
    end
  end
 ...
 case {'src-slice','src-slider'}
  ...
  if get(wgts.YorkCheck,'Value') > 0
    REF = getappdata(wgts.main,'REF');
    if size(SRC.dat,3) ~= size(REF.dat,3)
      T_IMAGE = getappdata(wgts.main,'T_IMAGE');
      if ~isempty(T_IMAGE.points) && any(T_IMAGE.points(1).slice)
        iSlice2 = round((iSlice-T_IMAGE.points(1).slice)*SRC.ds(3)/REF.ds(3)) + 1;
        %iSlice2 = floor((iSlice-T_IMAGE.points(1).slice)*SRC.ds(3)/REF.ds(3)) + 1;
      else
        iSlice2 = round((iSlice-1)*SRC.ds(3)/REF.ds(3)) + 1;
        %iSlice2 = floor((iSlice-1)*SRC.ds(3)/REF.ds(3)) + 1;
      end
    else
      iSlice2 = iSlice;
    end
    if iSlice2 > 0 && iSlice2 <= size(REF.dat,3)
      set(wgts.RefSliceEdt,'String',sprintf('%d',iSlice2));
      set(wgts.RefSldr,'Value',iSlice2);
      Main_Callback(wgts.main,'redraw-ref');
    end
  end
  Main_Callback(wgts.main,'redraw-src');
 ...
 case {'transform'}
  ...
  if any(strcmpi(METHOD,'none'))
    if length(T_IMAGE.points) < iSlice
      T_IMAGE.points(iSlice).refx = [];
      T_IMAGE.points(iSlice).refy = [];
      T_IMAGE.points(iSlice).srcx = [];
      T_IMAGE.points(iSlice).srcx = [];
      T_IMAGE.points(iSlice).slice = round(str2double(get(wgts.SrcSliceEdt,'String')));
    end
  end
  T_IMAGE.regimg(iSlice) = sub_transform(wgts,SRC,T_IMAGE,iSlice,'verbose',1,'method',METHOD);
  setappdata(wgts.main,'T_IMAGE',T_IMAGE);
...
switch lower(cmdstr)
 ...
 case {'replace x'}
  ...
  tmptitle = sprintf('%s: replace points',mfilename);
 ...
 case {'replace end'}
  Point_Function(wgts,'Remove END');
  Point_Function(wgts,'Append');
 ...
 case {'copy' 'copy points'}
  if isfield(T_IMAGE,'points') && length(T_IMAGE.points) >= iSlice
    CP_POINTS = T_IMAGE.points(iSlice);
    n = length(T_IMAGE.points(iSlice).refx);
    set(wgts.StatusField,'String',sprintf('copied %d points in the buffer.',n));
  else
    CP_POINTS = [];
    set(wgts.StatusField,'String','no points to copy to the buffer.');
  end
  drawnow;
  setappdata(wgts.main,'CP_POINTS',CP_POINTS);
  
 case {'paste to the current ref/src'}
  CP_POINTS = getappdata(wgts.main,'CP_POINTS');
  if isempty(CP_POINTS)
    set(wgts.StatusField,'String','no points to paste.');
  else
    T_IMAGE.points(iSlice) = CP_POINTS;
    T_IMAGE.points(iSlice).slice = round(str2double(get(wgts.SrcSliceEdt,'String')));
    if isfield(T_IMAGE,'regimg') && length(T_IMAGE.regimg) >= iSlice
      T_IMAGE.regimg(iSlice).img    = [];
      T_IMAGE.regimg(iSlice).dxy    = [];
    ...
    n = length(T_IMAGE.points(iSlice).refx);
    set(wgts.StatusField,'String',sprintf('%d points copied from the buffer.',n));
    setappdata(wgts.main,'T_IMAGE',T_IMAGE);
    Main_Callback(wgts.main,'redraw-ref');
    Main_Callback(wgts.main,'redraw-src');
    if any(DO_UPDATE_OVERLAY)
 ...
 case {'move to the current src'}
  if isfield(T_IMAGE,'points') && length(T_IMAGE.points) >= iSlice
    oldv = T_IMAGE.points(iSlice).slice;
    newv = round(str2double(get(wgts.SrcSliceEdt,'String')));
    T_IMAGE.points(iSlice).slice = newv;
    if isfield(T_IMAGE,'regimg') && length(T_IMAGE.regimg) >= iSlice
      T_IMAGE.regimg(iSlice).img    = [];
      T_IMAGE.regimg(iSlice).dxy    = [];
      T_IMAGE.regimg(iSlice).slice  = [];
    ...
    drawnow;
    n = length(T_IMAGE.points(iSlice).refx);
    set(wgts.StatusField,'String',sprintf('SLICE(%d) : npoints=%d SRC(%d-->%d)',iSlice,n,oldv,newv));
  end
...
%NEW_DATA.tform.tform        = [];  % 2019.09.12: don't keep, it may cause a problem between different matlab versions
...
elseif length(T_IMAGE.points) < iRefSlice || isempty(T_IMAGE.points(iRefSlice).refx)
  if VERBOSE
...
if any(strcmpi(METHOD,'none'))
  ...
  NEW_DATA.tform.method       = 'none';

  if VERBOSE
    StatusTxt = sprintf('%s no-transform done.',StatusTxt);
    set(wgts.StatusField,'String',StatusTxt);  drawnow;
  end
...
% MATLAB R2013a (8.1) started having 'imwarp'.
% 'linear conformal' is not supported in fitgeotrans() (R2019a)
if verLessThan('MATLAB','8.1') || any(strcmpi(METHOD,'linear conformal'))
  warpfuncname = 'imtransform';
  ...
  if VERBOSE
    StatusTxt = sprintf('%s fitgeotrans(''%s'').',StatusTxt,METHOD);
    set(wgts.StatusField,'String',StatusTxt);  drawnow;
  end
  switch lower(METHOD)
   case {'polynomial'}
    mytform = fitgeotrans(input_points,base_points,METHOD,3);  % default degree is 3 for cp2tform().
   case {'piecewise linear'}
    mytform = fitgeotrans(input_points,base_points,'pwl');
   case {'lwm'}
    mytform = fitgeotrans(input_points,base_points,METHOD,12); % default n-ponts is 12 for cp2tform().
   otherwise
    mytform = fitgeotrans(input_points,base_points,METHOD);
  end
  if VERBOSE
    StatusTxt = sprintf('%s imwarp()...',StatusTxt);
    set(wgts.StatusField,'String',StatusTxt);  drawnow;
  end 

  R = imref2d([ydata(2) xdata(2)]);
  if size(tmpimg,3) > 2
    tmpimg = permute(tmpimg,[2 1 3]);
    % note that tmpimg as (y,x,color)
    regimg = imwarp(tmpimg, mytform,'nearest','OutputView',R,'FillValues',[255;255;255]);
    regimg = permute(regimg,[2 1 3]);
  else
    tmpimg = tmpimg';
    % note that tmpimg as (y,x)
    regimg = imwarp(tmpimg, mytform,'nearest','OutputView',R,'FillValues',0);
    regimg = regimg';
  end
...
NEW_DATA.tform.func         = warpfuncname;
...
% NEW_DATA.tform.tform        = mytform;   % 2019.09.12: don't keep, it may cause a problem between different matlab versions
...
if length(T_IMAGE.regimg) >= iRefSlice && isfield(T_IMAGE.regimg(iRefSlice),'tform')
