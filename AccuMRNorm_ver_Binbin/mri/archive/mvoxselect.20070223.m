function ROITS = mvoxselect(Ses,GRPEXP,RoiName,MODEL,TRIAL,ALPHA)
...
%
%  EXAMPLE :
%   ROITS = mvoxselect('f05bf1','estim1','brain','corr[1]',[],0.01);
%
%   ROITS = 
%     session: 'f05bf1'
%     grpname: 'estim1'
...
%          ds: [0.7500 0.7500 2]
%          dx: 1
%         dat: [20x1064 double]  <-- time course (t,vox)
%        name: 'brain'
%      coords: [1064x3 double]
%         stm: [1x1 struct]
...
%       alpha: 0.0100
%     cluster: 1
%     datname: 'r'               <-- name of statistical value
%           p: [1064x1 double]   <-- p values
%         dat: [1064x1 double]   <-- statistical values
%
%  EXAMPLE-2 :
...
%
%  See also MVIEW MDIFFMAP MVOXSELECTMASK
...
ROITS.resp = subComputeResponse(ROITS);
...
function resp = subComputeResponse(ROITS)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
anap = getanap(ROITS.session,ROITS.grpname);
HemoDelay = anap.HemoDelay;
HemoTail  = anap.HemoTail;

iresp = getStimIndices(ROITS,'noblank',HemoDelay,HemoTail);
