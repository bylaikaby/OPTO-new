...
if isempty(WINDOW) || all(WINDOW(:) == 1),
  ...
  if VERBOSE,  fprintf(' window(%s).',WindowType);  end
...
if VERBOSE,  fprintf(' %s(nfft=%d).',METHOD,NFFT);  end
...
switch lower(METHOD),
 case {'fft'}
  ...
  SPC = 2*abs(SPC(tmpsel,:));  % single-sided
  F = F(tmpsel);
  
  % take care of DC and Fs/2
  tmpi = find(F == 0 | F == Fs/2);
  if any(tmpi),  SPC(tmpi,:) = SPC(tmpi,:)/2;  end
 ...
 case {'welch','pwelch'}
  ...
  for N = size(tcMrs.dat,2):-1:1,
    [Pxx, F] = pwelch(tcMrs.dat(:,N),[],[],NFFT,Fs);
    SPC(:,N) = sqrt(Pxx(:));
  end
  tmpsel = find(F <= Fs/2);
  SPC = SPC(tmpsel,:);
