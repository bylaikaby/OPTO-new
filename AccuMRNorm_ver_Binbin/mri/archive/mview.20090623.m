...
PickAreaCmb = uicontrol(...
    'Parent',hMain,'Style','Popupmenu',...
    'Units','char','Position',[XDSP+40 H 22 1.5],...
...
OverlayCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+108 H 20 1.5],...
...
uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+87 H-0.3 50 1.5],...
...
switch lower(eventdata),
 ...
 case {'slider-sagital'}
  ...
  if 0 && get(wgts.TCHoldCheck,'Value'),
    tmpimg = permute(get(GRAHANDLE.sagital,'cdata'),[2 1 3]) ;
  end
  if get(wgts.OverlayCheck,'Value'),
    ...
    if strcmpi(TrialNo,'all'),
      ...
      tmps(idx) = 0;
      tmpp(idx) = 1;
      tmpimg = subRoimerge('sagital',tmps,tmpp,MINV,MAXV,ALPHA,wgts,GRAHANDLE,tmpimg) ;
    end
  end
 ...
 case {'slider-coronal'}
  ...
  if get(wgts.OverlayCheck,'Value'),
    ...
    if strcmpi(TrialNo,'all'),
      ...
      tmps(idx) = 0;
      tmpp(idx) = 1;
      tmpimg = subRoimerge('coronal',tmps,tmpp,MINV,MAXV,ALPHA,wgts,GRAHANDLE,tmpimg) ;
    end
  end
 ...
 case {'slider-transverse'}
  ...
  if get(wgts.OverlayCheck,'Value'),
    ...
    if strcmpi(TrialNo,'all'),
      ...
      tmps(idx) = 0;
      tmpp(idx) = 1;
      tmpimg = subRoimerge('transverse',tmps,tmpp,MINV,MAXV,ALPHA,wgts,GRAHANDLE,tmpimg) ;
    end
  end
  ...
  for N = 1:length(SLICES),
    ...
    if ~isempty(STATMAP) & get(wgts.OverlayCheck,'Value') & ~strcmpi(TrialNo,'all'),
      ...
      newimage(N) = struct('tmps',tmps,'tmpp',tmpp,'MINV',MINV,'MAXV',MAXV,...
                           'ALPHA',ALPHA,'CMAP',CMAP,'iDimension',iDimension);
    ...
    if iDimension == 3,
      ...
      for K = 1:length(PICKAREA),
        if PICKAREA{K}.ipage == ipage,
          plot(PICKAREA{K}.anapx,PICKAREA{K}.anapy,'color',[0 1 0],'tag','PickArea');
        end
        if K == 1,
...
function CMAP = subGetColorMap(wgts,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if length(varargin)<1,
  ColorIndx = 1 ;
else
  ColorIndx = varargin{1} ;
end

colorcomb = {[1 0 0], [0 1 0], [0 0 1], [1 0 1], [1 1 0],[0 1 1]} ;
ROINO = mod(get(wgts.RoiCmb,'Value'),length(colorcomb)) + 1 ;
colorcomb = colorcomb{ROINO} ;
...
MINV = MINV(1) ;
if(MINV >= 0),
  for N = 1:length(colorcomb),
    if colorcomb(N),
      tmpcolormap(:,N) = colorcomb(N)*[0:255]/255 ;
    end
  end
else
  ...
  for N = 1:length(colorcomb),
    if colorcomb(N),
      tmpcolormap(1:lpart,N) = colorcomb(N)*[-lpart:-1]/lpart ;
    end
  end
  for N = 1:length(colorcomb),
    if colorcomb(N),
      tmpcolormap((1:upart)+lpart,N) = colorcomb(N)*[1:upart]/upart ;
    end
  end
end
...
if strcmpi(DatName,'Response'),
  if isfield(ROITS{1}{1},'xform') && isfield(ROITS{1}{1}.xform,'method'),
    switch lower(ROITS{1}{1}.xform.method),
     case {'tosdu', 'sdu'}
...
function tmpimg = subRoimerge(orient,tmps,tmpp,MINV,MAXV,ALPHA,wgts,GRAHANDLE,tmpimg)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
