...
if isfield(anap.mareats,'IRESAMPLE') && anap.mareats.IRESAMPLE,
...
switch lower(ModelStr),
 ...
 case { 'trialdthemo', 'dttrialhemo' }
  ...
  DT = diff(DAT);
  DAT(1:length(DT)) = DT;
 case { 'trialdtfhemo', 'dttrialfhemo' }
  trials = Channel;  Channel = [];
  HemoModel = 'fgampdf';
  ExpPar.stm = subUpdateStimVal(ExpPar,trials,[]);
  DAT = subGetBoxCar(ExpPar,IRDX,0,0,grp);
  ...
  DT = diff(DAT);
  DAT(1:length(DT)) = DT;
 case { 'trialdtvfhemo', 'dttrialvfhemo' }
  trials = Channel;  Channel = [];
  HemoModel = 'vfgampdf';
  ExpPar.stm = subUpdateStimVal(ExpPar,trials,[]);
  DAT = subGetBoxCar(ExpPar,IRDX,0,0,grp);
  ...
  DT = diff(DAT);
  DAT(1:length(DT)) = DT;
 case { 'trialdtcohen', 'dttrialcohen' }
  trials = Channel;  Channel = [];
  HemoModel = 'Cohen';
  ExpPar.stm = subUpdateStimVal(ExpPar,trials,[]);
  DAT = subGetBoxCar(ExpPar,IRDX,0,0,grp);
...
elseif ROI_LOAD == 0,
  ...
  if length(MODEL.dat) > LEN,
    MODEL.dat = MODEL.dat(1:LEN);
  elseif length(MODEL.dat) < LEN,
    L=LEN-length(MODEL.dat);
    MODEL.dat = cat(1,MODEL.dat,MODEL.dat(end-L+1:end));
  end
end
