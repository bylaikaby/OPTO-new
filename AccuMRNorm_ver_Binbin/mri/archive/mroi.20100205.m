...
RoiDrawCmb = uicontrol(...
    ...
    'String',{'all','current','markers','none(ana)','none'},'FontWeight','bold',...
...
if strcmpi(RoiDraw,'none(ana)'),  return;  end

IMGDIMS = getappdata(wgts.main,'IMGDIMS');
if any(strcmpi(RoiDraw,{'all','current'})),
  ...
  for N = 1:length(RoiRoi),
    ...
    if any(CurRoiName) && ~strcmpi(RoiRoi{N}.name,CurRoiName),  continue;  end
...
if ~any(strcmpi(RoiDraw,{'markers','none'})),
  for N = 1:length(RoiRoi),
    if RoiRoi{N}.slice ~= SLICE, continue;  end
    roiname = RoiRoi{N}.name;
...
if ~any(strcmpi(RoiDraw,{'none'}))
  ...
  for N = 1:length(RoiLR)
    ...
    axes(wgts.Image2Axs); hold on;
    plot(x,y,'color','r');
    text(x(1),y(1),'LR','color','y','fontsize',8);
  end
end
