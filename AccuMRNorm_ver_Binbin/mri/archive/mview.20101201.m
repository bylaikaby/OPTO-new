...
%    1.19 30.11.10 YM   supports SNR masking, color-map
%
...
MaskBySnrCheck = uicontrol(...
    ...
    'String','SNR-mask','FontWeight','bold',...
    'TooltipString','mask by SNR','BackgroundColor',get(hMain,'Color'));
MaskBySnrEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+147 H 25 1.5],...
    'Callback','mview(''Main_Callback'',gcbo,''edit-alpha'',guidata(gcbo))',...
    'String','50','Tag','MaskBySnrEdt',...
    'HorizontalAlignment','left',...
    'TooltipString','SNR for masking',...
    'FontWeight','Bold');
...
AnaScaleEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[XDSP+100 H 20.2 1.5],...
...
ScaleBarTxt = uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+130 H-0.3 20 1.5],...
 ...
 case {'init-stat'}
  ...
  switch lower(StatName),
   case {'corr'}
    ...
    if isfield(ROITS{1}{1},'snr') && ~isempty(ROITS{1}{1}.snr),
      set(wgts.DataCmb,'String',{'R_value','Response','SNR'},'Value',1);
    else
   ...
   case {'glm'}
    ...
    if isfield(ROITS{1}{1},'snr') && ~isempty(ROITS{1}{1}.snr),
      set(wgts.DataCmb,'String',{'StatV','beta','Response','SNR'},'Value',DATNAME_IDX);
    else
   ...
   otherwise
    ...
    if isfield(ROITS{1}{1},'snr') && ~isempty(ROITS{1}{1}.snr),
      set(wgts.DataCmb,'String',{'Response','SNR'},'Value',1);
    else
...
for N = 1:length(ROI.roi),
  ...
  if strcmpi(RoiName,'all') || strcmpi(roiroi.name,RoiName),
    ...
    hDR = plot(anax,anay,'color',tmpcol,'tag','ROI');
    if Visible,
      set(hDR,'Visible','on');
    else
      set(hDR,'Visible','off');
