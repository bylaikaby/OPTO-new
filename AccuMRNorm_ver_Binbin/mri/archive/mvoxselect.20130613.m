...
[MAP TC] = subGetData(Ses,GRPEXP,MODEL,TRIAL,ALPHA,RoiName,DO_CLUSTER,...
                      SIG,SIGNAME,SELECT_CORR,MIN_SNR,FDR_Q,VERBOSE);
...
function [STATMAP TC] = subGetData(Ses,GRPEXP,MODEL,TRIAL,ALPHA, RoiName, DO_CLUSTER, ROITS, SIGNAME, SELECT_CORR, MIN_SNR, FDR_Q, VERBOSE)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isempty(ROITS),
  ...
  ROITS = sigload(Ses,GRPEXP,SIGNAME);
else
...
%   switch lower(MODEL),
%    case { 'glm' }
%     STATMAP = subGetStatGLM(ROITS,ALPHA,RoiName,ModelNo,'statv',VERBOSE);
%    case { 'cor', 'corr' }
...
for T = 1:length(ROITS{1}),
  ...
  tmpdat = [];  tmpcoords = [];
  for N = 1:length(ROITS),
    if ~all(strcmpi(RoiName,'all')) && ~any(strcmpi(ROITS{N}{T}.name,RoiName)),  continue;  end
    if isempty(tmpdat),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBFUNCTION to generate statistical data correlation analysis
function STATMAP = subGetStatGLM(ROITS,alpha,RoiName,ModelNo,DATNAME,VERBOSE)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
STATMAP = {};
EPIDIM = size(ROITS{1}{1}.ana);
