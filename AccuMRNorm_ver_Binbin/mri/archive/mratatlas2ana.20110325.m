...
%
%  See also mratatlas2roi mratatlas2mng
%           anz_read anz_write spm_coreg rat16T_coreg rat16T_roi
...
if exist('Ses','var') && exist('GrpName','var'),
  ...
  if isfield(anap,'mratatlas2ana'),
    if isfield(anap.mratatlas2ana,'permute'),
      DO_PERMUTE = anap.mratatlas2ana.permute;
    end
    if isfield(anap.mratatlas2ana,'flipdim'),
      DO_FLIPDIM = anap.mratatlas2ana.flipdim;
    end
    if isfield(anap.mratatlas2ana,'minvoxels'),
      MIN_NUM_VOXELS = anap.mratatlas2ana.minvoxels;
    end
    if isfield(anap.mratatlas2ana,'twosteps'),
      DO_TWOSTEPS = anap.mratatlas2ana.twosteps;
    end
    if isfield(anap.mratatlas2ana,'spm_coreg'),
      FLAGS_SPM_COREG = anap.mratatlas2ana.spm_coreg;
    end
  end
end
...
fprintf('\n');
fprintf('ACCEPT LICENCE AGREEMENT OF RAT-ATLAS(AJ Schwartz,GlaxoSmithKline) AND SPM.\n');
