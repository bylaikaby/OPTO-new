...
%    1.27 06.07.17 YM   fixed problems of graphic handles (2014b).
%    1.30 19.04.18 YM   supports multiple testing correction (p-values adjust).
%    1.31 23.04.19 YM   supports exporting ana/epi as ANALYZE format.
%    1.32 24.04.19 YM   clean-up and supports exporting-stat.
%
%  See also ANALOAD ANAVIEW SETBACK SETFRONT MAREATS MROI GETSTIMINDICES MVOXSELECT PVAL_ADJUST
...
DrawEleCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+71 H 20 1.5],...
...
TestingCorrTxt =  uicontrol(...
    'Parent',hMain,'Style','Text',...
    'Units','char','Position',[XDSP+88 H-0.3 30 1.5],...
...
TestingCorrCmb = uicontrol(...
    ...
    'TooltipString','Multiple testing correction',...
...
OverlayCheck = uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+10+XSZ+15 H+YSZ-11.5 25 1.5],...
    'Tag','OverlayCheck','Value',1,...
    'Callback','mview(''Main_Callback'',gcbo,''redraw-image'',guidata(gcbo))',...
    'String','StatMap Overlay','FontWeight','bold',...
...
% Custom menu %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
m = uimenu(hMain,'Label','MROI');
uimenu(m,'Label','Redraw',...
    'Callback','mview(''Main_Callback'',gcbo,''redraw'',guidata(gcbo))');
uimenu(m,'Label','Save as Mask...','Separator','on',...
    'Callback','mview(''Main_Callback'',gcbo,''save-as-mask'',guidata(gcbo))');
uimenu(m,'Label','Export anatomy...','Separator','on',...
    'Callback','mview(''Main_Callback'',gcbo,''export-ana'',guidata(gcbo))');
uimenu(m,'Label','Export epi...',...
    'Callback','mview(''Main_Callback'',gcbo,''export-epi'',guidata(gcbo))');
uimenu(m,'Label','Export stat...',...
    'Callback','mview(''Main_Callback'',gcbo,''export-stat'',guidata(gcbo))');
...
switch lower(eventdata)
 ...
 case {'export-ana' 'export-epi'}
  if any(strcmpi(eventdata,'export-ana'))
    ANA  = getappdata(wgts.main,'ANA');
    fname = 'export-ana.hdr';
  else
    ANA  = getappdata(wgts.main,'EPIANA');
    fname = 'export-epi.hdr';
  end
  [f,d] = uiputfile( ...
	  {'*.img', 'All ANALYZE-7.5 Files (*.img)'; ...
       '*.nii', 'All NIfTI-1 Files (*.nii)'}, ...
	  'Save ANALYZE-7.5/NIfTI_1 file',fullfile(pwd,fname));
  if isequal(f,0) || isequal(d,0),  return;  end
  anzfile = fullfile(d,f);
  imgdim = [4 size(ANA.dat,1) size(ANA.dat,2) size(ANA.dat,3) 1];
  pixdim = [3 ANA.ds(1) ANA.ds(2) ANA.ds(3)];
  fprintf('imgdim[%s] pixdim[%s]', deblank(sprintf('%g ',imgdim(2:4))), deblank(sprintf('%g ',pixdim(2:4))));
 ...
 case {'export-stat'}
  ...
  fname = sprintf('export-stat(%s)_%s_p(%g).hdr',StatName,DATNAME,ALPHA);
  [f,d] = uiputfile( ...
	  {'*.img', 'All ANALYZE-7.5 Files (*.img)'; ...
       '*.nii', 'All NIfTI-1 Files (*.nii)'}, ...
	  'Save ANALYZE-7.5/NIfTI_1 file',fullfile(pwd,fname));
  ...
  fprintf('WARNING %s.Main_Callback(%s) is the experimental implementation (may have a problem).\n',mfilename,eventdata);
  fprintf('See also mstat2analyze()/mana2analyze().\n');

  EPIANA  = getappdata(wgts.main, 'EPIANA');
  STATMAP = getappdata(wgts.main, 'STATMAP');
  MINMAX  = getappdata(wgts.main, 'MINMAX');
  T = sscanf(TrialNo,'%d:');
  ...
  tmps = STATMAP{T}.dat;
  tmpp = STATMAP{T}.p;
  tmpm = STATMAP{T}.mask.dat;
  % mask by clustering/alpha etc
  idx = find(tmpm(:) == 0);
  tmps(idx) = 0;
  tmpp(idx) = 1;
  STATV.ds = EPIANA.ds;
  ...
  % NOTE: 3D-Slicer/Amira can read floating values ('single' and 'double').
  DO_SCALE = 0;
  if any(DO_SCALE)
    DataType = 'int16';
    glmax = intmax(DataType);
    % scale values
    fprintf(' scaling %s[%g %g] as [0 255]...',DATNAME,MINV,MAXV);
    MINV = MINMAX.(DATNAME)(1);  MAXV = MINMAX.(DATNAME)(2);
    STATV = (STATV - MINV)/(MAXV - MINV);
    STATV = round(STATV*255);
  else
    DataType = 'single';
    glmax = realmax(DataType); % might be useless since hdr.DIME.glmax should be int32.
  end
  
  imgdim = [4 size(STATV.dat,1) size(STATV.dat,2) size(STATV.dat,3) 1];
  pixdim = [3 STATV.ds(1) STATV.ds(2) STATV.ds(3)];
  fprintf(' imgdim[%s] pixdim[%s]', deblank(sprintf('%g ',imgdim(2:4))), deblank(sprintf('%g ',pixdim(2:4))));
  fprintf(' saving to ''%s''...',anzfile);
  hdr = hdr_init('dim',imgdim,'pixdim',pixdim,'datatype',DataType,'glmax',glmax);
  anz_write(anzfile,hdr,STATV.dat);
  fprintf(' done.\n');
...
for T = 1:length(ROITS{1})
  ...
  tmpmap.p = subAdjustPvalues(tmpmap.p,wgts);
...
function padj = subAdjustPvalues(porg,wgts)
% ==================================================================
% see https://www.rdocumentation.org/packages/stats/versions/3.4.3/topics/p.adjust
    ...
    if isfield(TCTRIAL{T},'stm') && ~isempty(TCTRIAL{T}.stm)
      ...
      for N = 1:length(stimv)
        ...
        for K = 1:length(hline)
          ...
          if any(ishandle(h1)) && any(ishandle(h2)),  break;  end
        end
        if isempty(h1)
          line([stimt(N),stimt(N)],ylm,'color','k','tag','stim-line');
        end
...
if ~any(ishandle(findobj(haxs,'tag','ScaleBar')))
...
if length(get(hsrc,'UserData')) > 1
  ...
  tmpstr = sprintf('%s P%s<%s Model=%s/%s',tmpstr,padjtxt,get(wgts.AlphaEdt,'String'),...
                   StatName,ModelNo);
else
...
tmpstr2 = sprintf('%s P%s<%s ROI=%s Model=%s/%s',tmpstr2,padjtxt,get(wgts.AlphaEdt,'String'),...
                  RoiName,StatName,ModelNo);
if length(get(wgts.TrialCmb,'String')) > 1
  tmpstr2 = sprintf('%s Trial=%s',tmpstr2,TrialNo);
end
