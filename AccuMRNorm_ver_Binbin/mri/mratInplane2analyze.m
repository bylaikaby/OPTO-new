function mratInplane2analyze(Ses,GrpName,varargin)
...
%    0.93 16.01.12 YM  adds photoshop-raw info in txt.
%    0.94 04.07.13 YM  uses matlas_defs(), supports sesversion()>=2.
%    0.95 17.07.13 YM  uses expfilename() instead of catfilename().
%
%  See also mratatlas2roi m_inplaneAnatomy2analyze matlas_defs mratraw2img hdr_init anz_write
...
if LOAD2DSEQ,
  ...
  if ~isempty(ASCAN),
    if sesversion(Ses) >= 2,
      ANA = load(sigfilename(Ses,grp.ana{2},grp.ana{1}),grp.ana{1});
      ANA = ANA.(grp.ana{1});
    else
...
if ~isempty(FLIP_DIM),  fprintf(' flipping dim.');  end
...
if ImgDistort,
  tmpimgfile = sprintf('tcImg-%s',expfilename(Ses,grp.exps(1),'2dseq'));
else
