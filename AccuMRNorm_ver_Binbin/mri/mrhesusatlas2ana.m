...
%    GRP.xxx.anap.mrhesusatlas2ana.atlas     = 'CoCoMac(brain)';
...
%    0.95 03.02.12 YM  use sigiflename() instead of catfilename().
%    0.96 04.07.13 YM  use matlas_defs().
%
%  See also anz_read anz_write spm_coreg mcoreg_spm_coreg matlas_defs
%           matlas2roi mratatlas2ana mratatlas2mng
...
VA = spm_vol(fullfile(INFO.template_dir,INFO.template_file));
...
ROITABLE = matlas_roitable(fullfile(INFO.template_dir,INFO.table_file));
switch lower(INFO.type)
 case {'cocomac'}
  atlas = load(fullfile(INFO.template_dir,INFO.atlas_file),'ATLAS');
  atlas = atlas.ATLAS;
