...
CreateROICheck = uicontrol(...
    ...
    'String','Create Rat-ROI on SAVE','FontWeight','bold',...
...
switch lower(eventdata),
 ...
 case {'load-atlas'}
  ...
  if strncmpi(ses.name,'rat',3),
    setappdata(wgts.main,'IS_RAT_ATLAS',1);
    % rat atlas
    ...
    if exist(afile,'file'),
      ...
      nrois = length(unique(tmpatlas.dat(:)));
      ...
      switch lower(atype)
       case {'coronal ap' 'coronal pa'}
        for N = 1:size(tmpatlas.dat,3),
          ...
          ATLAS(N).map = cmap;
          ATLAS(N).res = [0.1 0.1];
          ATLAS(N).coords = (N-1)*0.1;  % str2double() returns incorrect value...
          % ATLAS(N).coords = N;
        end
        if strcmpi(atype,'coronal pa'),  ATLAS = ATLAS(end:-1:1);  end
       case {'horizontal dv' 'horizontal vd'}
        for N = 1:size(tmpatlas.dat,2),
          ATLAS(N).img = squeeze(tmpatlas.dat(:,N,:))';
          ATLAS(N).map = cmap;
          ATLAS(N).res = [0.1 0.1];
          ATLAS(N).coords = (N-29)*0.1;  % str2double() returns incorrect value...,  29 is
                                         % cortical surface
          % ATLAS(N).coords = N;
        end
        if strcmpi(atype,'horizontal vd'),  ATLAS = ATLAS(end:-1:1);  end
      end
    ...
    % monkey atlas
    setappdata(wgts.main,'IS_RAT_ATLAS',0);
 ...
 case {'save-atlas'}
  ...
  if get(wgts.CreateROICheck,'Value') > 0 && getappdata(wgts.main,'IS_RAT_ATLAS') > 0,
...
if getappdata(wgts.main,'IS_RAT_ATLAS') > 0,
  % no RGB conversion...
else
...
if size(tmpimg,3) > 2,
  ...
  regimg  = imtransform(tmpimg, mytform,'nearest','xdata',xdata,'ydata',ydata,'FillValues',0);
end
