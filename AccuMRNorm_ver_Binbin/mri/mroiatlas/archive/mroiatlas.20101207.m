...
%    3)  "SAVE" button saves transformed atlas as "(grproi)_atlas" in "atlas_tform.mat".
%
%  EXAMPLE :
%    mroiatlas('i07541')
%
...
AtlasSelCmb = uicontrol(...
    ...
    'Tag','AtlasSelCmb','FontWeight','Bold');
AtlasSliceEdt = uicontrol(...
    'Parent',hMain,'Style','Edit',...
    'Units','char','Position',[IMGXOFS+IMGW+37, H 7 1.5],...
...
switch lower(eventdata),
 ...
 case {'load-ana'}
  ...
  set(wgts.AtlasSliceEdt, 'String', sprintf('%d',1));
  ...
  if exist(tfile,'file'),
    ...
    set(wgts.StatusField,'String',StatusTxt');  drawnow;
    vname = sprintf('%s_atlas',grp.grproi);
  ...
  for K = 1:length(T_ATLAS.points)
    if any(T_ATLAS.points(K).atlas_name),
      tmpidx = find(strcmpi(get(wgts.AtlasSelCmb,'String'),T_ATLAS.points(K).atlas_name));
      if any(tmpidx),
        set(wgts.AtlasSelCmb,'Value',tmpidx(1));
        break;
      end
    end
  end
 ...
 case {'load-atlas'}
  % loading atlas
  atype = get(wgts.AtlasSelCmb,'String');
  atype = atype{get(wgts.AtlasSelCmb,'Value')};
  afile = fullfile(fileparts(which(mfilename,'fullpath')),'mroiatlas','atlas.mat');
  ATLAS = [];  ATLAS_COORDS = [];
  if exist(afile,'file'),
    ...
    N = length(ATLAS);
    set(wgts.AtlasSliceEdt, 'String', sprintf('%+d',ATLAS(1).coords));
  ...
  if ~isempty(T_ATLAS) && ~isempty(ATLAS),
    ...
    if isfield(T_ATLAS,'points') && length(T_ATLAS.points) >= iSlice
      ...
      if any(tmpcoords),
        tmpidx = find(ATLAS_COORDS == tmpcoords(1));
        if any(tmpidx),
          set(wgts.AtlasSliceEdt, 'String', sprintf('%+d',tmpcoords(1)));
 ...
 case {'atlas-slice','atlas-slider'}
  ...
  if strcmpi(eventdata,'atlas-slice')
    ...
    ATLAS_COORDS = getappdata(wgts.main,'ATLAS_COORDS');
    set(wgts.AtlasSliceEdt,'String',sprintf('%+d',ATLAS_COORDS(iSlice)));
  end
  Main_Callback(wgts.main,'redraw-atlas');
  Main_Callback(wgts.main,'redraw-overlay');
  ...
  if length(T_ATLAS.points) >= iSlice
    ...
    if any(tmpcoords),
      ...
      if any(tmpidx),
        ...
        tmptxt = sprintf('%s(%+g)',T_ATLAS.points(iSlice).atlas_name,tmpcoords(1));
      end
    end
  end
 ...
 case {'transform'}
  ...
  if ~isempty(T_ATLAS.atlas(iSlice).img) && ~any(T_ATLAS.atlas(iSlice).coords == tmpcoords),
    ...
    tmpidx = find(ATLAS_COORDS == tmpcoords);
    if any(tmpidx),
 ...
 case {'point-action'}
  ...
  set(wgts.AtlasSelCmb,   'Enable','off');
  try
  ...
  set(wgts.AtlasSelCmb,   'Enable','on');
...
if length(T_ATLAS.points) < iSlice,
  T_ATLAS.points(iSlice).atlas_name = '';
...
switch lower(cmdstr),
 case {'append'}
  ...
  atlas_name = get(wgts.AtlasSelCmb,'String');
  atlas_name = atlas_name{get(wgts.AtlasSelCmb,'Value')};
  NEW_POINTS.atlas_name = atlas_name;
...
tmpidx = find(ATLAS_COORDS == PINFO.coords);
if isempty(tmpidx),
...
if VERBOSE,
  StatusTxt = sprintf('SLICE(%d) : npoints=%d atlas=%s(%+g).',...
                      iAnaSlice,size(input_points,1),PINFO.atlas_name,PINFO.coords(1));
  set(wgts.StatusField,'String',StatusTxt);  drawnow;
end
...
if 1,
  ...
  sx = floor(ANA.ds(1)/ATLAS(iAnaSlice).res(2));
  sy = floor(ANA.ds(2)/ATLAS(iAnaSlice).res(1));
...
regimg  = imtransform(tmpimg, mytform,'xdata',xdata,'ydata',ydata,'FillValues',[255;255;255]);
...
if ATLAS(iSlice).coords ~= points.coords,  return;  end
...
ATLAS = getappdata(wgts.main,'ATLAS');
tmpcoords = round(str2double(get(wgts.AtlasSliceEdt,'String')));
...
if isempty(NEW_ATLAS.img) || ~isequal(NEW_ATLAS.coords,tmpcoords),
  % use the current
