function [Sig corana] = mcorana(varargin)
...
%
%  VERSION :
%    1.00 NKL, 01.13.00, 07.10.01, 02.09.02, 23.10.02 17.04.04
%    1.01 NKL, 27.12.05, 05.01.06
%    1.02 24.03.06 YM  modified so that this can be called from catsig, checked with n03ow1.
%    1.03 22.05.07 YM  bug fix when anap.gettrial.status > 0
%    1.04 16.06.08 YM  support mdlsct{}.bold_tfilter as [hpassHz, lpassHz]
%    1.05 15.10.08 YM  set 'DX' for expmkmodel().
...
if RUN_matscor > 0,
  if iscell(Sig{1}),
    [Sig refTs corana] = subDo_troiTs(Ses,GRPEXP,grp,anap,Sig);
  else
    [Sig corana] = subDo_roiTs(Ses,GRPEXP,grp,anap,Sig);
  end
  if nargout == 1,
    Sig = sub_update_stat(Sig,corana);
  end
else
...
function [troiTs refTs corana] = subDo_troiTs(Ses,ExpNo,grp,anap,troiTs)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(anap.gettrial,'trial2obsp') & anap.gettrial.trial2obsp > 0,
  ...
  for M = 1:length(grp.corana),
    ...
    mdlsct{M} = expmkmodel(Ses,ExpNo,grp.corana{M}.mdlsct,'DX',troiTs{1}.dx(1));
    if isfield(grp.corana{M},'bold_tfilter'),
      if iscell(mdlsct{M}),
        for A = 1:length(mdlsct{M}),
          mdlsct{M}{A}.bold_tfilter = grp.corana{M}.bold_tfilter;
        end
      else
        mdlsct{M}.bold_tfilter = grp.corana{M}.bold_tfilter;
      end
    end
    if isfield(grp.corana{M},'lagfix'),
      if iscell(mdlsct{M}),
        for A = 1:length(mdlsct{M}),
          mdlsct{M}{A}.lagfix = grp.corana{M}.lagfix;
        end
      else
        mdlsct{M}.lagfix = grp.corana{M}.lagfix;
      end
    end
    if isfield(grp.corana{M},'tselect'),
      if iscell(mdlsct{M}),
        for A = 1:length(mdlsct{M}),
          mdlsct{M}{A}.tselect = grp.corana{M}.tselect;
        end
      else
        mdlsct{M}.tselect = grp.corana{M}.tselect;
      end
    end
  end;

  fprintf(' matscor(shift=%g).',anap.shift);
  [troiTs corana] = matscor(troiTs,mdlsct,anap.shift);
  %fprintf(' done.\n');
  ...
  for M = 1:length(grp.corana),
    fprintf('%s.',grp.corana{M}.mdlsct);
    tmp = expmkmodel(Ses,ExpNo,grp.corana{M}.mdlsct,'PreT',PreT,'PostT',PostT,'DX',troiTs{1}{1}.dx(1));
    if isfield(grp.corana{M},'bold_tfilter'),
      if iscell(tmp),
        for A = 1:length(tmp),
          tmp{A}.bold_tfilter = grp.corana{M}.bold_tfilter;
        end
      else
        tmp.bold_tfilter = grp.corana{M}.bold_tfilter;
      end
    end
    if isfield(grp.corana{M},'lagfix'),
      if iscell(tmp),
        for A = 1:length(tmp),
          tmp{A}.lagfix = grp.corana{M}.lagfix;
        end
      else
        tmp.lagfix = grp.corana{M}.lagfix;
      end
    end
    if isfield(grp.corana{M},'tselect'),
      if iscell(tmp),
        for A = 1:length(tmp),
          tmp{A}.tselect = grp.corana{M}.tselect;
        end
      else
        tmp.tselect = grp.corana{M}.tselect;
      end
    end
  ...
  if ~isempty(TrialIndex) & TrialIndex > 0,
    ...
    for R = 1:length(troiTs),
      ...
      [tmpTs tmpcorana] = matscor(tmpTs,mdlsct{R}{T},anap.shift);
      corana{R}{T}     = tmpcorana{1};
    end
    ...
    for R = 1:length(troiTs),
      for T = 1:length(troiTs{R}),
        ...
        corana{R}{T} = corana{R}{TrialIndex};
      end
    end
...
function [roiTs corana] = subDo_roiTs(Ses,ExpNo,grp,anap,roiTs)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for M=1:length(grp.corana)
  fprintf('%s.',grp.corana{M}.mdlsct);
  mdlsct{M} = expmkmodel(Ses,ExpNo,grp.corana{M}.mdlsct,'DX',roiTs{1}.dx(1));
...
if isstim(Ses,grp.name) || any(neumdl),
  fprintf(' matscor(shift=%g)...',anap.shift);
  [roiTs corana] = matscor(roiTs,mdlsct,anap.shift);
else
  fprintf(' cleaning ./.p...');
  for R = 1:length(roiTs),
    corana{R}.session = roiTs{R}.session;
    corana{R}.grpanme = roiTs{R}.grpname;
    corana{R}.ExpNo   = roiTs{R}.ExpNo;
    corana{R}.ana     = roiTs{R}.ana;
    corana{R}.r{1}    = zeros(size(roiTs{R}.dat,2),1,'single');
    corana{R}.p{1}    = ones(size(roiTs{R}.dat,2),1,'single');
  end
end
fprintf(' done.\n');
...
% ==================================================================
function Sig = sub_update_stat(Sig,corana)
% ==================================================================
if iscell(Sig),
  for N = 1:length(Sig),
    Sig{N} = sub_update_stat(Sig{N},corana{N});
  end
  return
end
