function varargout = mcor(x,y,nlags,lagfix)
...
%  
% NKL, 18.07.04
% YM,  16.06.08  support lagfix
% YM,  16.02.11  potential bug fix, when all(x == x(1)).
%
...
if ~exist('lagfix','var'),  lagfix = [];  end


if nlags || ~isempty(lagfix),
  mode = 3;
else
  mode = 1;
end;
...
switch mode,
 ...
 case 3,                            % Doing explicitly corrcoeff's job
  ...
  if 0,
    % xcorr() as xcorr(OUT,IN) or xcorr(BOLD,MODEL)
    x = rand(100,2);
    x(1:20,:) = 0.5;
    x([1:80],1) = x([1:80]+10,2);
    [c lags] = xcorr(x(:,2),x(:,1),20,'coef');
    figure;
  ...
  VOXIDX = find(any(y,1));
   ...
   if isempty(lagfix),
     %maxv = zeros(size(dx));
     for N=VOXIDX,
       [tmpr,lags] = xcorr(y(:,N),x,nlags,'coef');
       [mx,mxi] = max(abs(tmpr));
       dx(N) = lags(mxi);
       %maxv(N) = tmpr(mxi);
     end;
   ...
   for N=VOXIDX,
     if dx(N) > 0,
       % + lags, shift y leftward by dx
       sy = nanbuf;
       sy(1:end-dx(N)) = y(1+dx(N):end,N);
       [tmpr,tmpp] = corrcoef(x(:),sy,'rows','pairwise');
     else
       % - lags, shift x leftward by dx
       sx = nanbuf;
       sx(1:end+dx(N)) = x(1-dx(N):end);
