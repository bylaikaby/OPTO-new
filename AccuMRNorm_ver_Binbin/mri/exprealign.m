...
%    0.94 19.01.12 YM  supports 'spont' imaging.
%    0.95 04.07.12 YM  use sigfilename() than catfilename().
%    0.96 05.06.13 YM  save with -v7.3.
%
...
if DO_EXPORT,
  ...
  [fp,fr] = sigfilename(Ses,ExpNo,'mat');
...
if DO_REALIGN,
  ...
  fprintf(' %s: spm_realign() making alignment data (quality=%.2f,fwhm=%.2f,sep=%.2f,PW=''%s'')...',...
          gettimestring,flags.quality,flags.fwhm,flags.sep,flags.PW);
  spm_realign(P,flags);
...
tcImg.spm = FLAGS;
save(sigfilename(Ses,ExpNo,'tcImg'),'tcImg','-v7.3');
clear tcImg;
...
if isfield(grp,'daqver') && grp.daqver >= 2,
  ...
  if length(spar.trial.tonset{1}) > 1,
    tcImg = sigsort(tcImg,spar.trial,[],[],0,0);
    if iscell(tcImg),  tcImg = tcImg{1};  end
  else
    % likely 'spont' activity, create 'fake' trials.
    if ~isfield(tcImg,'centroid') || isempty(tcImg.centroid),
    ...
    % tmpdat = tcImg.dat;
    % for iZ = 1:size(tmpdat,3),
    %   for N = 1:size(tmpdat,4),
    %     tmpdat(:,:,iZ,N) = edge(tmpdat(:,:,iZ,N),'canny');
    %   end
    % end
    % cent = mcentroid(tmpdat,tcImg.ds);
    
    cent = cent';  % (xyz,t) --> (t,xyz)
    for N = 1:3,
      cent(:,N) = cent(:,N) - nanmean(cent(:,N));
    end
    % should be stable for all XYZ
    tmpthr = nanstd(cent,[],1) * 0.7;
    tmpidx = find(abs(cent(:,1)) < tmpthr(1) & ...
                  abs(cent(:,2)) < tmpthr(2) & ...
                  abs(cent(:,3)) < tmpthr(3));
    tmpidx = tmpidx(1:floor(length(tmpidx)/4)*4);
    tcImg.dat = tcImg.dat(:,:,:,tmpidx);
    tmpsz = size(tcImg.dat);
    tcImg.dat = reshape(tcImg.dat,[tmpsz(1:3) tmpsz(4)/4 4]); % (x,y,z,t,trials)
  end
end

% set NaN as 0
tcImg.dat(isnan(tcImg.dat(:))) = 0;
