...
%    0.92 04.04.07 YM  use ACQP_grad_matrix/ACQ_phase1_offset
%
...
fprintf('%s: Slice position is by guesss from ACQP. If wrong, use ParaVision instead.\n',mfilename);

% FLIP DIMENSION, (:,:,1)=transv, (:,:,2)=sagital, (:,:,3)=coronal
TRIPILOT.dat(:,:,1) = TRIPILOT.dat(:,end:-1:1,1);
TRIPILOT.dat(:,:,2) = TRIPILOT.dat(end:-1:1,:,2);
TRIPILOT.dat(:,:,2) = TRIPILOT.dat(:,end:-1:1,2);
TRIPILOT.dat(:,:,3) = TRIPILOT.dat(:,end:-1:1,3);
...
if ~isempty(GRP),
  ...
  if isfield(acqp,'ACQ_fov'),
    tmptxt = deblank(sprintf('%g ',acqp.ACQ_fov));
  ...
  if isfield(acqp,'ACQ_slice_angle'),
    tmptxt = deblank(sprintf('%g ',acqp.ACQ_slice_angle));
    infotxt{end+1} = sprintf('ACQ_slice_angle= %s',tmptxt);
  end
  if isfield(acqp,'ACQ_phase1_offset'),
    tmptxt = deblank(sprintf('%g ',acqp.ACQ_phase1_offset));
    infotxt{end+1} = sprintf('ACQ_phase1_offset= [%s]',tmptxt);
  end
  if isfield(acqp,'ACQ_phase2_offset'),
    tmptxt = deblank(sprintf('%g ',acqp.ACQ_phase2_offset));
    infotxt{end+1} = sprintf('ACQ_phase2_offset= [%s]',tmptxt);
  end
  if isfield(acqp,'ACQ_slice_thick'),
    infotxt{end+1} = sprintf('ACQ_slice_thick= %g mm',acqp.ACQ_slice_thick);
  end
  if isfield(acqp,'ACQ_slice_sepn'),
    tmptxt = deblank(sprintf('%g ',acqp.ACQ_slice_sepn));
    infotxt{end+1} = sprintf('ACQ_slice_sepn= [%s] mm',tmptxt);
  end
  if isfield(acqp,'ACQ_slice_offset'),
    tmptxt = deblank(sprintf('%g ',acqp.ACQ_slice_offset));
    infotxt{end+1} = sprintf('ACQ_slice_offset= [%s] mm',tmptxt);
  end
  % RECO parameters
  if isfield(reco,'RECO_fov'),
    tmptxt = deblank(sprintf('%g ',reco.RECO_fov));
...
set(haxs,'ydir','normal','xdir','reverse');
...
ph1offs = acqp.ACQ_phase1_offset;
ph2offs = acqp.ACQ_phase2_offset;

%slreadoffs = acqp.ACQ_read_offset;

if isfield(acqp,'ACQ_grad_matrix'),
  gradmat = acqp.ACQ_grad_matrix;
else
  fprintf('WARNING %s: ''acqp.ACQ_grad_matrix'' is missing, please run sesdumppar() again.\n',mfilename);
  gradmat = [];
end
...
haxs = wgts.SagitalAxs;
axes(haxs);
tmpw = fov(2);  tmph = slithk;
for N = 1:length(isodist),
  x0 = -tmpw/2;
  y0 = isodist(N) - tmph/2;
  if ~isempty(gradmat),
    tmpmat = squeeze(gradmat(N,[2 3],[2 3]));
    % NOTE THAT cw/ccw as +/- for ParaVision
    tmpmat = tmpmat .* [1 -1; -1 1];
    
    tmppos = [x0 y0;  x0+tmpw y0;  x0+tmpw y0+tmph;  x0 y0+tmph]';
    tmppos = tmpmat * tmppos;
    tmppos(:,end+1) = tmppos(:,1);
    
    tmpoffs(1) =  ph1offs(N)*tmpmat(1,1);
    tmpoffs(2) = -ph1offs(N)*tmpmat(1,2);
    
    %line(tmppos(1,:),tmppos(2,:),'color',RECTCOLOR,'tag','slices');
    line(tmppos(1,:)+tmpoffs(1),tmppos(2,:)+tmpoffs(2),'color',RECTCOLOR,'tag','slices');
  else
    rectangle('Position',[x0 y0 tmpw tmph],...
