...
ACHXFORM    = 0;
if isfield(anap.showmap,'ACHXFORM') & anap.showmap.ACHXFORM,
  ACHXFORM = 1;
end;

YLIM=[];
if isfield(anap.showmap,'YLIM'),
  YLIM = anap.showmap.YLIM;
end;
...
for N=1:2:length(varargin),
  switch lower(varargin{N}),
   ...
   case {'lcutoff'}
    LCUTOFF = varargin{N+1};
   case {'hcutoff'}
    HCUTOFF = varargin{N+1};
   case {'ygrid'},
    YGRID = varargin{N+1};
...
[roiTs, roiTXT] = selroits(SesName, GrpName, varargin{:});
...
if ischar(GrpName),
  ...
  % GrpName as ExpNumber
  title(sprintf('showmap(%s,%d)', upper(SesName), GrpName));
end
axis off;
...
% subplot('position',POS);
POS=[2 4 6 8 10 12 14];
if ACHXFORM,
  roiTs = achxform(roiTs);
end;
for N=1:length(roiTs),
  ...
  if exist('LineStyle','var') && ~isempty(LineStyle),
    if LineStyle(N)
      hd(N) = dsproits(roiTs{N},'color',COL_LINE(N),'bstrp',BSTRP,'cival',...
                       CIVAL,'linewidth',1.5,'stderror',STDERROR,'ylim',YLIM);
    else
      hd(N) = dsproits(roiTs{N},'color',COL_LINE(N),'bstrp',0,'cival',CIVAL,'linewidth',2,...
                       'ylim',YLIM,'linestyle',':');
    end;
  else
    hd(N) = dsproits(roiTs{N},'color',COL_LINE(N),'bstrp',BSTRP,'cival',...
