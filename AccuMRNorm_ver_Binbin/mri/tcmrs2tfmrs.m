...
%    'window'   : window type, 'none'|'hanning'|'hamming'|'blackman'|'kaiser'|'blackmanharris'
%    'winsize'  : window size  (for pwelch)
%    'noverlap' : overlap size (for pwelch)
...
% ANALYSIS OPTIONS 
METHOD     = 'fft';  % fft|pwelch
WindowType = 'none'; % window type
WindowSize = [];     % window size for pwelch
NOVERLAP   = [];     % num. overlap for pwelch
NFFT       = [];     % num. fft size
Nskip      = 0;      % skip leading data points
Pack2blp   = 0;
Bands      = {};
DoSave     = 0;
VERBOSE    = 1;
...
if isfield(anap,'tcmrs2tfmrs')
  ...
  if isfield(anap.tcmrs2tfmrs,'window')
    WindowType = anap.tcmrs2tfmrs.window;
  end
  if isfield(anap.tcmrs2tfmrs,'winsize')
    WindowSize = anap.tcmrs2tfmrs.winsize;
  end
  if isfield(anap.tcmrs2tfmrs,'nwin')
    WindowSize = anap.tcmrs2tfmrs.nwin;
  end
  if isfield(anap.tcmrs2tfmrs,'noverlap')
    NOVERLAP = anap.tcmrs2tfmrs.noverlap;
  end
...
for N = iopt:2:length(varargin)
  switch lower(varargin{N})
   ...
   case {'winsize' 'windowsize' 'nwindow' 'nwin'}
    WindowSize = varargin{N+1};
   case {'noverlap' 'overlap'}
    NOVERLAP = varargin{N+1};
   case {'nfft'}
    NFFT = varargin{N+1};
   case {'nskip'}
    Nskip = varargin{N+1};
...
if any(Nskip)
  fprintf(' len(%d->%d).',size(tcMrs.dat,1),size(tcMrs.dat,1)-Nskip);
...
switch lower(METHOD),
 ...
 otherwise
  if isempty(WindowSize),
    % use the same way as pwelch() does
    M = size(tcMrs.dat,1);
    if isempty(NOVERLAP)
      % Use 50% overlap
      WindowSize = fix(M./4.5);
      NOVERLAP = fix(0.5.*WindowSize);
    else
      WindowSize = fix((M+7.*NOVERLAP)./8);
    end
    clear M;
  end
end
...
switch lower(METHOD),
 case {'fft'}
  ...
  if isempty(WINDOW) || all(WINDOW(:) == 1),
    ...
    tcMrs.dat = bsxfun(@times, tcMrs.dat, WINDOW(:));
 ...
 case {'welch','pwelch'}
  ...
  if VERBOSE,
    fprintf(' %s(nfft=%d,win=%s(%d),overlap=%d).',METHOD,NFFT,WindowType,length(WINDOW),NOVERLAP);
  end
  SPC = [];
  for N = size(tcMrs.dat,2):-1:1,
    [Pxx, F] = pwelch(tcMrs.dat(:,N),WINDOW,NOVERLAP,NFFT,Fs);
    SPC(:,N) = Pxx(:);
  end
