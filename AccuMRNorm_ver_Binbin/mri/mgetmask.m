function varargout = mgetmask(Ses,GrpExp,SAVE2MAT,SigName)
...
%    0.96 28.02.11 YM   supports SigName,grp.glmsigs.
%    0.97 31.01.12 YM   use sigfilename().
%
...
if sesversion(Ses) >= 2,
  matfile = sigfilename(Ses,GrpExp,'mask');
  mmkdir(fileparts(matfile));
else
  matfile = sigfilename_ver1(Ses,GrpExp,'mat');
end
if SAVE2MAT > 0,
  % make mask data and save it, usually called from sesgetmask().
  mask = subCreateMask(Ses,GrpExp,SigName);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function mask = subCreateMask(Ses,GrpExp,SigName)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
try
  mask = subCreateCorrMask(Ses,GrpExp,SigName);
catch
  mask = [];
end
if isempty(mask)
  mask = CreateGlmMask(Ses,GrpExp,SigName);
else
...
function MASK = subCreateCorrMask(Ses,GrpExp,SigName)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isempty(SigName),
  if isfield(grp,'glmsigs') && ~isempty(grp.glmsigs),
    SigName = grp.glmsigs;
    if iscell(SigName),  SigName = SigName{1};  end
  else
