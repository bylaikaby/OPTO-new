...
%  phase correction, PSF correction.  The original 2dseq is renamed as 2dseq.bru
%
...
% phase correction
if any(PHASE_CORR),
  IMG = epi_phcorr(IMG,'iteration',PHASE_N_ITER,'verbose',VERBOSE);
end
...
if any(PSF_CORR),
  ...
  PSF_PATH = fullfile(fileparts(fileparts(fileparts(fileparts(TDSEQFILE)))),num2str(PSF_SCAN));
  IMG = epi_psfcorr(IMG,PSF_PATH,'shiftsize',SHIFT_SIZE,'threshold',MASKING_THR,'upsample',UPSAMPLE_V,'verbose',VERBOSE);
end
...
if isfloat(IMG),
  ...
  switch reco.RECO_wordtype,
   case {'_8BIT_UNSGN_INT'}
    ...
    IMG = uint8(IMG / maxv * single(rawMax) );
   case {'_16BIT_SGN_INT'}
    %   IMG = int16(IMG);
    %  IMG = int16(IMG / maxv * single(intmax('int16')) );
    IMG = int16(IMG / maxv * single(rawMax) );
   case {'_32BIT_SGN_INT'}
    %   IMG = int32(IMG);
    %  IMG = int32(IMG / maxv * single(intmax('int32')) );
    IMG = int32(IMG / maxv * single(rawMax) );
  end
end
