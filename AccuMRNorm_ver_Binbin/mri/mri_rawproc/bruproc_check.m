function varargout = bruproc_check(varargin)
%BRUPROC_CHECK - GUI check for post-processing of Bruker 2dseq.
%  BRUPROC_CHECK run a GUI for DB's 
%
%  EXAMPLE :
%    >> bruproc_check('setup','4.7T')
%    >> bruproc_check('setup','7T')
%
%  VERSION :
%    0.90 05.04.19 YM  pre-release
%
%  See also bruproc bruproc_gui

persistent H_BRUPROC_CHECK;  % keep the figure handle
...
if ishandle(H_BRUPROCCHECK)
  close(H_BRUPROCCHECK);
  %fprintf('\n ''mgui'' already opened.\n');
  %return;
end
...
uicontrol(...
    ...
    'Callback','bruproc_check(''Main_Callback'',gcbo,''browse-datadir'',guidata(gcbo))',...
...
uicontrol(...
    'Parent',hMain,'Style','PushButton',...
    'Units','char','Position',[XDSP H 12.5 1.5],...
    'Callback','bruproc_check(''Main_Callback'',gcbo,''browse-study'',guidata(gcbo))',...
...
uicontrol(...
    ...
    'Callback','bruproc_check(''Main_Callback'',gcbo,''pvpar'',guidata(gcbo))',...
...
uicontrol(...
    ...
    'HorizontalAlignment','left',...
    'TooltipString','PSF threshold: frac. of max(std)',...
    'FontWeight','Bold','FontName','FixedWidth');
uicontrol(...
    'Parent',hMain,'Style','Text',...
...
uicontrol(...
    'Parent',hMain,'Style','Checkbox',...
    'Units','char','Position',[XDSP+29 H 25 1.5],...
    'Tag','PSFExtrapolateCheck','Value',DEF.psf_extrapolate,...
    'String','Extrapolate shift-map','FontWeight','bold',...
    'TooltipString','PSF extrapolate shift-map','BackgroundColor',get(hMain,'Color'));
...
uicontrol(...
    ...
    'Callback','bruproc_check(''Main_Callback'',gcbo,''undo'',guidata(gcbo))',...
...
uicontrol(...
    ...
    'Callback','bruproc_check(''Main_Callback'',gcbo,''run'',guidata(gcbo))',...
...
switch lower(eventdata)
 ...
 case {'run'}
  ...
  psf_maxshift = get(wgts.PSFMaxShiftEdt,'String');
  psf_extrapolate = get(wgts.PSFExtrapolateCheck,'Value');
  perm_v     = get(wgts.PermuteEdt,'String');
  flip_v     = get(wgts.FlipdimEdt,'String');
  proc_prec  = get(wgts.ProcPrecisionCmb,'String');  proc_prec = proc_prec{get(wgts.ProcPrecisionCmb,'Value')};
  ...
  for K = 1:length(SCANS)
    ...
    cmdstr = [ 'bruproc(''' DATAPATH '''' ',' scan ',' reco ...
               ...
               ',''PSFMaxShift'','         psf_maxshift ...
               ',''PSFExtrapolate'','      num2str(psf_extrapolate) ...
               ',''Permute'',' '['   perm_v ']' ...
               ',''FlipDim'',' '['   flip_v ']' ...
               ',''precision'',' '''' proc_prec '''' ...
