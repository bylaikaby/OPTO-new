...
%    1.13 07.10.13 NKL Resample before filtering, MB_METHOD, detrend...
%    1.14 19.11.13 NKL potential bug fix on normalization caused by IREMBRAINMEAN.
%    1.15 19.15.17 YM  skips when nt=1.
%
...
DEF.IREMBRAINMEAN       = 0;        % Remove brain-mean from all roiTs
DEF.ISUBSTITUDE         = 0;
DEF.IRESAMPLE           = 0;
DEF.IDETREND            = 1;
...
DEF.IROIFILTER          = 0;        % Spatial filter with ROI masking, see roifilt2()
...
% =========================================================================================
% roiTs and froiTs have entirely different preprocessing, defined in ANAP.mareats.PARS...
% =========================================================================================
...
if exist('GrpName','var'),
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % As of now we'll have two distinctly different BOLD time series (both existed by w/ overlap)
  % roiTs - NO PREPROCESSING AT ALL; the data are just converted from tcImg with MTIMESERIES
  % froiTs - Re-alignment, removal of brain-mean (PC1) and Spatiotemporal Filtering
  % NKL-YM (11.12.2018)
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if size(tcImg.dat,4) == 1,
  roiTs = {};    IsChanged = 0;
  fprintf(' WARNING: no time series (nt=%d), skipped...\n',size(tcImg.dat,4));
  return;
end
...
%
% OLD CODE (to be entirely deleted from documentationl...)
...
if ICONCAT,
  ...
  % PRACTICALLY NEVER USED.... (SLICE-ROIs are discrete!)
  for RoiNo=1:length(Roi.roi),
    if any(strcmpi(IEXCLUDE,Roi.roi{RoiNo}.name)), continue;
    end;
    roiTs{ValidRoiNo} = rts;
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EXAMPLE(roiTs Extracted at date: '10-Dec-2018', time: '12:58:21')
%           IEXCLUDE: {'Brain'  'LEFT'  'RIGHT'}
%            ICONCAT: 1  [all slice-parts of a ROI]
%           IMIMGPRO: 1  Filtering etc.
%      IFILTER_KSIZE: 3
%         IFILTER_SD: 1.2500
%             INOTCH: 1
%        ICUTOFFHIGH: 0.0200
%        COMPUTE_SNR: 1
%             ITOSDU: {'zerobase'  'blank'}

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MB 06.10.2013 - Implementing MB's idea for normalization
% ------------------------------------------------------------------------------------------
% We (MB and NKL) talked on Sunday evening and decided to not use the mean-subtraction. It
% works without biasing the data in rats, but it does eliminate a lot of cortical responses
% in monkeys. As an alternative we'll eliminate the "first" PC component of all the
% brain. Since the elimination requires exactly the same voxels for the sum of ROIs and the
% "Brain", instead of using the "Brain" ROI, we concatenate the individual ROIs...
% NKL 07.10.2013
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
MB_METHOD = 1;
% DETREND   - Otherwise the first PC is mostly the DC offset
% SVDS      - Extract the first PC and save singular values, u(m,k), the diagonal s(k,k),
%             and the scales v(n,k).
% MEAN      - save also the mean of the entire brain
% ------------------------------------------------------------------------------------------
% BRAINMEAN(1) - To continue below...
if IREMBRAINMEAN && MB_METHOD,
  for N=1:length(roiTs), roiTs{N}.dat = detrend(roiTs{N}.dat); end;
  DC_removed = 1;
  
  dat = []; for N=1:length(roiTs), dat = cat(2, dat, roiTs{N}.dat); end;
  nyq=1/(2*roiTs{1}.dx);  Fcut=.1;
  [b,a] = butter(8,Fcut/nyq,'low'); 
  mriSmooth=filtfilt(b,a,dat);
  mriRough=dat-mriSmooth;
  [BrainRoi.u, BrainRoi.s, BrainRoi.v] = svds(mriRough,1);
  BrainRoi.dx = roiTs{1}.dx;
  BrainRoi.dat = nanmean(dat,2);
end;
% ATTENTION(Removal of mean continues below after some preprocessing !!!!!)
% ------------------------------------------------------------------------------------------
...
% BRAINMEAN(CONTINUATION FROM THE BRAINMEAN(1)
if  IREMBRAINMEAN,
  if MB_METHOD,
    fprintf(' rembase[-svds(1)].');
    y = BrainRoi.u * BrainRoi.s * BrainRoi.v';
    for N=1:length(roiTs),
      roiTs{N}.dat = bsxfun(@minus, roiTs{N}.dat, y(:,1:size(roiTs{N}.dat,2)));
      y(:,1:size(roiTs{N}.dat,2))=[];
    end;
  else
    fprintf(' rembase[-mean].');
    for N=1:length(roiTs),
      roiTs{N}.dat = bsxfun(@minus, roiTs{N}.dat, BrainRoi.dat);
    end;
  end;
end;
...
% Respiration at 60 strokes/min, i.e. 1 Hz (RespFr) [Checked with MB, 12.09.2013]
% Until 12.09.2013 TR = 1 seconds, Sampling Freq = 1Hz, Nyquist = 0.5 Hz
% Expected interference AliasedRespFr = NyqF - (RespFr - NyqF)
% ResidualRespFr = None
% AliasedRespFr = 0.5 - (1 - 0.5) = 0 Hz... (None)
% NO visible interference in the data
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% REMOVE RESPIRATION ARTIFACTS AND LOWPASS FITLER BY USING INVERTING THE FFT-SPECTRUM
% THIS IS NOT USED ANYMORE... but just leave it as is...  NKL 12.09.2013
if IFFTFLT,
  fprintf(' matsfft.');
...
% RESPIRATION ARTIFACTS REMOVAL (PROJECTING OUT SINUSOIDS)
if any(IARTHURFLT),
...
% RESPIRATION ARTIFACTS REMOVAL (NOTCH)
if INOTCH,
  fprintf(' RespRemNotch.');
  NW = 2; NFFT = 1024;
  for N=1:length(roiTs),
    tmpdat = nanmean(roiTs{N}.dat,2);
    [tmppxx(:,N), F] = pmtm(tmpdat,NW,NFFT,1/roiTs{N}.dx);
  end;
  if roiTs{1}.dx < 1,
    FrIndex = find(F > 0.25  & F < 0.45);
    FrRange = F(FrIndex);
  else
    FrIndex = find(F > 0.075 & F < 0.125);
    FrRange = F(FrIndex);
  end;
  for N=1:length(roiTs)
    [mx, tmpidx] = max(tmppxx(FrIndex,N),[],1);
    tmpf = FrRange(tmpidx);
    roiTs{N} = sigfiltfilt(roiTs{N},tmpf,'notch');
  end;
end;

% RESPIRATION ARTIFACTS REMOVAL (PCA)
if ~isempty(IRESPBAND),
  NFFT = 1024;
  IRESPBAND_DEBUG = 0;
  fprintf(' RespRemPCA[%.3f-%.3f]-%g).', IRESPBAND, IRESPTHR);
  for AreaNo = 1:length(roiTs),
    ...
    s(ArtIdx, ArtIdx) = 0;
    if IRESPBAND_DEBUG, hold off; plot(nanmean(roiTs{AreaNo}.dat,2)); end;
    roiTs{AreaNo}.dat = u * s * v';
    if IRESPBAND_DEBUG, hold on; plot(nanmean(roiTs{AreaNo}.dat,2),'r'); keyboard; end;
  end;
  fprintf('.');
end;
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% RESAMPLING FIRST WORKS BETTER (LESS ARTIFACTS....)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [pxx, F] = getpsd(Sig)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
PNTLEN    = round(size(Sig.dat,1)/4);
NFFT      = 2^nextpow2(PNTLEN);
NOVERLAP  = round(PNTLEN/4);
Sig = xform(Sig,'zerobase');
for N=1:size(Sig.dat,2),
  [pxx(:,N), F] = pwelch(Sig.dat(:,N), PNTLEN, NOVERLAP, NFFT, 1/Sig.dx);
end;
return;
