...
%    SNR           : minimum SNR
%    FDRQ          : q value for False-Discovery-Rate, "ALPHA" will be updated.
%    AdjustPvalues : adjust p values for multiple testing correction: 'none','Bonferroni','Holm','Hochberg','BY','fdr','Sidak'.
%
%  EXAMPLE :
...
%       trial: 1
%       alpha: 0.0100
%   uncorrected_alpha: 0.0100
%     cluster: 1
%     datname: 'f'               <-- name of statistical value
%           p: [2170x1 double]   <-- p values
%         dat: [2170x1 double]   <-- statistical values
%       fdr_q: 0.05              <-- optional argument (Q value for FDR)
%       fdr_p: 0.004             <-- calculated p value with the given Q
%
%   ROITS.resp =
%    stimtype: 'anystim'
%    basetype: 'prestim'
...
%    1.03 27.10.10 YM  looks for glm-name in roiTs{X}.glmcont first.
%    1.04 02.12.10 YM  supports '.snr'.
%    1.05 01.02.12 YM  adds '.beta' for GLM
%    1.06 05.03.12 YM  supports snr selection.
%    1.07 14.07.12 YM  supports FDR calculation.
%    1.08 18.07.12 YM  supports FDR calculation/selection with updated ALPHA.
%    1.09 14.12.12 YM  returns the available statistics even when alpha >=1.
%    1.10 30.04.13 YM  bug fix, do averaging when ndims(sig.dat) > 2.
%    1.11 13.06.13 YM  gives an error when the given 'model' is not in data.
%    1.20 06.11.13 YM  improved speed, x2.1 faster.
%    1.21 19.04.18 YM  supports multiple testing correction ('adjust_pvalues').
%    1.22 13.12.18 YM  supports user-statistics (.stat) for selection.
%
%  See also mview mdiffmap mvoxlogical mvoxresponse dspmvox dspmvoxmap dspmvoxtc FDR_thres pval_adjust
...
ADJ_PVAL = 'none';
% cluster detection
if isfield(anap,'mview') && isfield(anap.mview,'cluster'),
DO_CLUSTER = anap.mview.cluster;
else
DO_CLUSTER = 1;
end
...
for N = optidx:2:length(varargin),
  switch lower(varargin{N}),
   ...
   case {'snr' 'minsnr'}
    MIN_SNR = varargin{N+1};
   case {'fdrq' 'fdrqval' 'fdr_q' 'fdr_qval' 'fdr'}
    FDR_Q = varargin{N+1};
   case {'adjustp' 'padjust' 'adjustpvalues' 'pvaluesadjust' 'adjust_pvalues' 'pvalues_adjust'}
    ADJ_PVAL = varargin{N+1};
   case {'verbose','verb'}
...
if ~isempty(ADJ_PVAL) && ~strcmpi(ADJ_PVAL,'none'),
  if exist('pval_adjust.m','file') ~= 2,
    fprintf(' WARNING %s: ''pval_adjust.m'' not found, disabled multiple testing correction.\n',mfilename);
    ADJ_PVAL = 'none';
  end
  if any(FDR_Q),
    error(' ERROR %s: options(FDRQ and AdjustPvalues) must be mutually exclusive.\n');
  end
end
...
if isempty(SIG),
  ...
  SIG = sigload(Ses,GRPEXP,SIGNAME);
end
% PROCESS DATA
[MAP TC] = subGetData(Ses,grp,anap,SIG,MODEL,TRIAL,ALPHA,RoiName,DO_CLUSTER,...
                      SELECT_CORR,MIN_SNR,FDR_Q,ADJ_PVAL,VERBOSE);

if isempty(MAP)
  if VERBOSE
    fprintf('  WARNING %s: Given ROIs (%s) are not found.\n',mfilename,sub_roi_str(RoiName));
  end
  ROITS = [];
  return
end
...
mapidx = find(MAP.mask.dat(:) > 0);
[voxidx tcsel mapsel] = intersect(TC.sub2ind, mapidx);
mapidx = mapidx(mapsel);
[ix,iy,iz] = ind2sub(size(MAP.dat),voxidx);
coords     = [ix(:),iy(:),iz(:)];
...
if any(strcmpi(MODEL,{'userstat' 'user-stat' 'stat' 'user' 'usr'}))
  if isfield(MAP,'model'),
    ROITS.stat.model   = MAP.model;
  else
    ROITS.stat.model   = 'user-stat';
  end
end
ROITS.stat.trial   = TRIAL;
ROITS.stat.alpha   = MAP.mask.alpha;
ROITS.stat.uncorrected_alpha = ALPHA;
ROITS.stat.fdr_q   = FDR_Q;
ROITS.stat.fdr_p   = MAP.mask.fdr_p;
ROITS.stat.datname = MAP.datname;
ROITS.stat.p       = MAP.p(mapidx);
ROITS.stat.dat     = MAP.dat(mapidx);
if isfield(MAP,'beta') && ~isempty(MAP.beta)
  % for GLM(statv)
  ROITS.stat.beta  = MAP.beta(mapidx);
end
ROITS.stat.mask    = rmfield(MAP.mask,'dat');
ROITS.stat.adjust_pvalues = MAP.adjust_pvalues;
ROITS.stat.cluster = MAP.mask.cluster;  % redundant, but keep it for compatibility
...
%ROITS.resp = subComputeResponse(ROITS,anap,StimType);
...
% ==============================================================================
function  [Ses grp] = sub_sig_sesgrp(SIG)
% ==============================================================================

if iscell(SIG)
  [Ses, grp] = sub_sig_sesgrp(SIG{1});
  return
end
...
function [STATMAP TC] = subGetData(Ses,grp,anap,ROITS,MODEL,TRIAL,ALPHA, RoiName, DO_CLUSTER, SELECT_CORR, MIN_SNR, FDR_Q, ADJ_PVAL, VERBOSE)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if ~exist('DO_CLUSTER','var'),  DO_CLUSTER = 1;  end
if ~exist('ROITS','var'),       ROITS = [];      end
if ~exist('SELECT_CORR','var'), SELECT_CORR = 'pos';  end
if ~exist('MIN_SNR','var'),     MIN_SNR = 0;     end
if ~exist('VERBOSE','var'),     VERBOSE = 1;     end

% make ROITS compatible to troiTS
if isstruct(ROITS),  ROITS = { ROITS };  end
if iscell(ROITS) && isstruct(ROITS{1}),
...
% ROITS_IDX
ROITS_SEL = zeros(1,length(ROITS));
for N = 1:length(ROITS)
  if ~all(strcmpi(RoiName,'all')) && ~any(strcmpi(ROITS{N}{1}.name,RoiName)),  continue;  end
  ROITS_SEL(N) = 1;
end
ROITS = ROITS(ROITS_SEL > 0);
if isempty(ROITS),
  STATMAP = [];  TC = [];
  return
end
...
% if ndims(.dat) > 2 (ie multiple trials/obs), then average
for N = 1:length(ROITS),
  for T = 1:length(ROITS{N}),
    if ndims(ROITS{N}{T}.dat) > 2
      tmpsz = size(ROITS{N}{T}.dat);
      ROITS{N}{T}.dat = reshape(ROITS{N}{T}.dat,[tmpsz(1) tmpsz(2) prod(tmpsz(3:end))]);
      ROITS{N}{T}.dat = nanmean(ROITS{N}{T}.dat,3);
    end
  end
end
...
[MODEL ModelNo] = subGetModel(Ses,grp,MODEL,ROITS{1}{1});
...
switch lower(MODEL),
 case { 'glm' }
  STATMAP = subGetStatGLM(anap,ROITS,ALPHA,ModelNo,'statv',ADJ_PVAL,VERBOSE);
 case { 'cor', 'corr' }
  STATMAP = subGetStatCorr(anap,ROITS,ALPHA,ModelNo,SELECT_CORR,ADJ_PVAL);
 case { 'none', 'null' }
  STATMAP = subGetStatNull(ROITS,ALPHA);
  DO_CLUSTER = 0;
 case { 'user-stat' }
  STATMAP = subGetUserStat(ROITS,ALPHA,ADJ_PVAL);
 otherwise
  if ALPHA >= 1,
    STATMAP = subGetStatNull(ROITS,ALPHA);
    DO_CLUSTER = 0;
...
% SNR selection
STATMAP.mask.min_snr = MIN_SNR;
if any(MIN_SNR) && isfield(ROITS{1}{1},'snr') && ~isempty(ROITS{1}{1}.snr),
  STATMAP.mask.dat(ROITS{1}{1}.snr(:) < MIN_SNR) = 0;
end


% FDR_Q selection
STATMAP.mask.fdr_q = FDR_Q;
STATMAP.mask.fdr_p = NaN;
STATMAP.mask.uncorrected_alpha = STATMAP.mask.alpha;
if any(FDR_Q)
  tmpp = STATMAP.p(STATMAP.p(:) < ALPHA);
  STATMAP.mask.fdr_p = FDR_thres(tmpp(:)',FDR_Q);
  STATMAP.mask.alpha = STATMAP.mask.fdr_p;
  STATMAP.mask.dat(STATMAP.p(:) >= STATMAP.mask.fdr_p) = 0;
end
...
for T = 1:length(ROITS{1}),
  ...
  if isfield(ROITS{1}{T},'ele'),
    TC{T}.ele = ROITS{1}{T}.ele;
  end;
  
  if isfield(ROITS{1}{T},'snr')
    TC{T}.snr   = ROITS{1}{T}.snr;
  end
  TC{T}.coords  = [];
...
function [ModelName ModelNo] = subGetModel(Ses,grp,ModelStr,ROITS)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isempty(tmpc)
  if any(strcmpi(ModelStr,{'none','null',''})),
    ModelName = 'none';
    ModelNo   = [];
  elseif any(strcmpi(ModelStr,{'userstat' 'user-stat' 'stat' 'user' 'usr'})),
    ModelName = 'user-stat';
    ModelNo   = [];
  elseif ~any(strcmpi({'glm','corr'},ModelStr)),
    [ModelName ModelNo] = subGetModel(Ses,grp,sprintf('glm[%s]',ModelStr),ROITS);
  else
    ModelName = ModelStr;
    ModelNo = 1;
  end
...
if isfield(grp,'refgrp') && ~isempty(grp.refgrp) && ~isempty(grp.refgrp.grpexp) && ~strcmp(grp.refgrp.grpexp,grp.name),
  ...
  sname = signame(ROITS);
  if isfield(grp,'glm') && isfield(grp.glm,sname) && isfield(grp.glm.(sname),'glmconts')
    glmconts = grp.glm.(sname).glmconts;
  elseif isfield(grp,'glmconts'),
    glmconts = grp.glmconts;
  end
  if isfield(grp,'corana') && isfield(grp.corana,sname) && isfield(grp.corana.(sname),'glmconts')
    corana = grp.glm.(sname).corana;
  elseif isfield(grp,'corana')
    corana = grp.corana;
  end
end;
...
switch lower(ModelName),
 case {'glm'}
  % GLM analysis
  % first look for ROITS.glmcont().cont.name
  if isfield(ROITS,'glmcont') && ~isempty(ROITS.glmcont),
    for C = 1:length(ROITS.glmcont)
      if isfield(ROITS.glmcont(C),'cont'),
        if strcmpi(ROITS.glmcont(C).cont.name,tmpname),
          ModelNo = C;
          break;
        end
      end
  ...
  % % now check grp.glmcont, this may be changed after GLM analysis...
  % if isempty(ModelNo),
  ...
  if ~any(ModelNo)
    ...
    if isfield(ROITS,'glmcont') && ~isempty(ROITS.glmcont),
      for C = 1:length(ROITS.glmcont)
        tmptxt = [tmptxt ROITS.glmcont(C).cont.name ','];
      end
      tmptxt = tmptxt(1:end-1);
    end
    error('\n ERROR %s: ''%s'' is not found in data.\n  sig.glmcont: %s.\n',mfilename,ModelStr,tmptxt);
  end
...
function resp = subComputeResponse(ROITS,anap,StimType)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function STATMAP = subGetStatCorr(anap,ROITS,alpha,ModelNo,SELECT_CORR,ADJ_PVAL)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
STATMAP = {};
EPIDIM = size(ROITS{1}{1}.ana);
DATNAME = 'r';
...
for T = 1:length(ROITS{1}),
  ...
  tmpmap.p = subAdjustPvalues(tmpmap.p,ADJ_PVAL);
  tmpmap.adjust_pvalues = ADJ_PVAL;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBFUNCTION to generate statistical data correlation analysis
function STATMAP = subGetStatGLM(anap,ROITS,alpha,ModelNo,DATNAME,ADJ_PVAL,VERBOSE)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
STATMAP = {};
EPIDIM = size(ROITS{1}{1}.ana);
...
for T = 1:length(ROITS{1}),
  ...
  tmpmap.dat     = zeros(EPIDIM);
  tmpmap.p       = ones(EPIDIM);
  tmpmap.beta    = zeros(EPIDIM);
  tmpmap.coords  = [];
  tmpmap.datname = DATNAME;
  for N = 1:length(ROITS),
    MX = max(ROITS{N}{T}.glmcont(ModelNo).selvoxels);
    if MX > size(ROITS{N}{T}.coords,1),
      fprintf('ROI Name = %s\n', ROITS{N}{T}.name);
      fprintf('size(.coords,1) = %d\n', size(ROITS{N}{T}.coords));
      fprintf('Maximum index of selected voxels (.selvoxels): %d\n', MX);
      keyboard;
    end;
    ...
    try
      %tmpmap.dat      = zeros(EPIDIM,class(tmpV));
      %tmpmap.p        = ones(EPIDIM,class(tmpP));
      tmpmap.dat(idx) = tmpV(:);
      tmpmap.p(idx)   = tmpP(:);
    catch
      keyboard
    ...
    if any(strcmpi(DATNAME,{'statv' 'stat'})),
      % adds 'beta' if possible
      if isfield(ROITS{N}{T}.glmcont(ModelNo),'BetaMag') && ~isempty(ROITS{N}{T}.glmcont(ModelNo).BetaMag),
        tmpB = ROITS{N}{T}.glmcont(ModelNo).BetaMag;
      else
        if VERBOSE && ~strcmpi(ROITS{N}{T}.glmcont(ModelNo).cont.type,'f'),
          fprintf(' WARNING %s: no .glmcont{}.BetaMag...\n',mfilename);
        end;
        % no way...
        tmpB = zeros(1,length(ROITS{N}{T}.glmcont(ModelNo).statv));
      end
      tmpmap.beta(idx) = tmpB(:);
    end
  ...
  tmpmap.mask.dat     = zeros(EPIDIM,'int8');
  tmpmap.mask.dat(tmpmap.p(:) < alpha) = 1;
  if any(strcmpi(DATNAME,{'beta'})),
    tmpmap = rmfield(tmpmap,'beta');
  end
  STATMAP{T} = tmpmap;
end
...
function STATMAP = subGetStatNull(ROITS,alpha)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
STATMAP = {};
EPIDIM = size(ROITS{1}{1}.ana);
for T = 1:length(ROITS{1}),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBFUNCTION to generate statistical data for 'user-stat'
function STATMAP = subGetUserStat(ROITS,alpha,ADJ_PVAL)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
STATMAP = {};
EPIDIM = size(ROITS{1}{1}.ana);
for T = 1:length(ROITS{1}),
  ...
  %tmpmap         = ROITS{1}{T}.stat;
  tmpfields = fieldnames(ROITS{1}{T}.stat);
  for K = 1:length(tmpfields),
    tmpmap.(tmpfields{K}) = ROITS{1}{T}.stat.(tmpfields{K});
  end
  
  tmpmap.dat     = zeros(EPIDIM);
  tmpmap.p       = ones(EPIDIM);
  tmpmap.coords  = [];
  if ~isfield(tmpmap,'datname')
    tmpmap.datname = 'user-stat';
  end
  ...
  for N = 1:length(ROITS),
    ...
    try
      tmpmap.dat(idx) = ROITS{N}{T}.stat.dat(:);
      tmpmap.p(idx)   = ROITS{N}{T}.stat.p(:);
    catch
      keyboard
    end
  end
...
function padj = subAdjustPvalues(porg,method)
% ==================================================================
...
switch lower(method),
 case {'none'}
  padj = porg;
 case {'bonferroni'}
  padj = porg * numel(porg);
 case {'bonferroni-holm' 'holm'}
  padj = pval_adjust(porg,'holm');
 case {'hochberg'}
  padj = pval_adjust(porg,'hochberg');
 case {'hommel'}
  % this tooks forever...
  padj = pval_adjust(porg,'hommel');
 case {'by' 'benjamini-yekutieli'}
  padj = pval_adjust(porg,'BY');
 case {'bh(fdr)' 'bh' 'fdr' 'benjamini-hochberg'}
  padj = pval_adjust(porg,'fdr');
 case {'sidak'}
  padj = pval_adjust(porg,'sidak');
 otherwise
  error(' ERROR %s: unknown method(%s) for pval_adjust.m',mfilename,method);
...
% ======================================================================
function RoiStr = sub_roi_str(RoiName)
% ======================================================================
if ischar(RoiName),  RoiName = { RoiName };  end
RoiStr = RoiName{1};
for N = 2:min(5,length(RoiName))
  RoiStr = [RoiStr '+' RoiName{N}];
end

if length(RoiName) > 5, RoiStr = [RoiStr '...'];  end
