...
for N = NoObsp:-1:1,
  ...
  if N == 1,
    ...
    for k=1:length(fnames),
      etimeE{N}.(fnames{k}) = subSubtractMRI1E(etime{N}.(fnames{k}),t0);
    end
    etimeE{N}.mri1E = t0;
  else
...
if strcmpi(ExpEvt.system,'MriFixate'),
  ExpEvt = sub_fix_MriFixate(ExpEvt);
end
...
if isfield(grp,'jawpo') & ~isempty(grp.jawpo),
  ...
  elseif strcmpi(SRC,'dgz'),
    ...
    if CHN(1) > 0,
      jawpo.dat(:,1) = ems{CHN(1)}(:);
    end
    if length(CHN) >= 2 & CHN(2) > 0,
      tmpdat = ems{CHN(2)};
      if isfield(jawpo,'dat') & ~isempty(jawpo.dat),
        if size(jawpo.dat,1) > length(tmpdat),
          tmpdat(end+1:size(jawpo.dat,1)) = 0;
        else
          tmpdat = tmpdat(1:size(jawpo.dat,1));
        end
      end
      jawpo.dat(:,2) = tmpdat(:);
    else
    ...
    if 1,
      % bug fix
      jawpo.dx  = jawpo.dx * 2;
      jawpo.dat = jawpo.dat(1:ceil(size(jawpo.dat,1)/2),:);
    end
    if ObspNo == 1,
      jawpo.dat = jawpo.dat(max([1 round(mri1E(1)/1000/jawpo.dx)]):end,:);
    end
  end
...
for N = 3:2:length(DG.e_pre),
  ...
  switch lower(DG.e_pre{N}{2}),
   case {'-numtriggerspervolume'}
    PAR.numTriggersPerVolume =  str2num(DG.e_pre{N+1}{2});
   case {'-intervolumetime (ms)'}
    PAR.interVolumeTime_ms = str2num(DG.e_pre{N+1}{2});
   case {'-dummy triggers'}
    PAR.dummyTriggers = str2num(DG.e_pre{N+1}{2});
   case {'-mri trigger'}
    PAR.mriTrigger = str2num(DG.e_pre{N+1}{2});
   case {'reset stim.sequence'}
    PAR.resetStimSequence = str2num(DG.e_pre{N+1}{2});
   case {'adfw host'}
    PAR.adfwHost = deblank(DG.e_pre{N+1}{2});
   case {'save adfw file'}
    PAR.saveADFWFile = str2num(DG.e_pre{N+1}{2});
   case {'save adfw file 2'}
    PAR.saveADFWFile2 = str2num(DG.e_pre{N+1}{2});
   case {'pulse on obsp/stimon'}
    PAR.pulseOnObspStimon = str2num(DG.e_pre{N+1}{2});
   case {'show debug info'}
    PAR.showDebugInfo = str2num(DG.e_pre{N+1}{2});
   ...
   case {'f post-stim. fix radius'}
    PAR.fixRadiusPostStim = str2num(DG.e_pre{N+1}{2});
  end
end
...
for k=1:length(fnames),
  evt.obs{1}.times.(fnames{k}) = subSubtractMRI1E(evt.obs{1}.times.(fnames{k}),t0);
end
