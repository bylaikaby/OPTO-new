...
%    1.01 18.11.15 YM  supports reading by MEX without SON2.
%
%  See also expgetpar sesdumppar expfilename smr_read smrmat2Cln sigsave
...
for K = 1:length(SPIKE2.stim),
  ...
  if strcmpi(SMRFILE(end-3:end),'.smr')
    warning(' WARNING %s: import of stimuli is not supported yet.',mfilename);
    return
    % marker or event?
    %tmpstim = smr_read(SMRFILE,SPIKE2.stim{K},'window_sec',Ses.expp(ExpNo).smrwin);
    %tmpstim = smr_read(SMRFILE,SPIKE2.stim{K},'window_sec',Ses.expp(ExpNo).smrwin);
    tmpstim.times = tmpstim.times - SMR_OFFS_SEC;
  else
