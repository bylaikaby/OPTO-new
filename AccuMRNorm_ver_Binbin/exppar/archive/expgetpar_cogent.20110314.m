...
%
%  See also expgetpar sesdumppar catfilename
...
LOGFILE = catfilename(Ses,ExpNo,'cogentlog');
COGENT  = grp.COGENT;
...
if isnifti(grp),
  [fp fr fe] = fileparts(catfilename(Ses,ExpNo,'nifti'));
  tmpdirs = dir(catfilename(Ses,ExpNo,'nifti'));

  if isempty(tmpdirs),
    fprintf('\n ERROR %s: NIFTI files not found, check "EXPP(%d).nifti".',mfilename,ExpNo);
    fprintf('\n    EXPP(%d).nifti = ''%s''.\n',strrep(catfilename(Ses,ExpNo,'nifti'),'\','/'));
    keyboard
  end
