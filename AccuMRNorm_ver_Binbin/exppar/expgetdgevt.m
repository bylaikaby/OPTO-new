function [ExpEvt, DG] = expgetdgevt(Ses, ExpNo)
%EXPGETDGEVT - Uses adf_info/dg_read to get all events of experiment ExpNo
% EXPEVT = EXPGETDGEVT(SES,EXPNO) gets recorded events for SES/EXPNO.
% EXPEVT = EXPGETDGEVT(DGZFILE)
% NKL, 4.10.02
%
% NOTES : 03.02.04  YM
...
%   2.04 27.05.19 YM  clean-up.
%   2.05 29.05.19 YM  supports EXPP().ShiftedMriTrigger also.
%   2.10 12.11.19 YM  renamed as expget"dg"evt from expgetevt to avoid name-conflict.
%   2.11 12.11.19 YM  use select"dg"evt/prm, get"dg"evtcodes to avoid name-conflict.
%
% See also EXPGETPAR, GOTO, GETSES, EXPFILENAME, GETGRP, GETDGEVTCODES
...
if ~any(DG.e_types{NoObsp}==46) && ~any(strfind(Ses.name,'ymfs'))
  NoObsp = NoObsp - 1;
  if NoOsbp == 0
    fprintf('WARNING %s: No true/pesudo MRI events (system=%s)',mfilename,NoObsp,DG.e_pre{1}{2});
    NoObsp = 1;
  end
end
...
NumTriggers = selectdgprm(DG, 1, ec.ObspType, 1);
...
for N = NoObsp:-1:1
  etime{N}.begin  = selectdgevt(DG, N, ec.BeginObsp,  ec.sub.all);
  etime{N}.end	  = selectdgevt(DG, N, ec.EndObsp,    ec.sub.all);
  etime{N}.isi    = selectdgevt(DG, N, ec.Isi,		ec.sub.all);
  etime{N}.ttype  = selectdgevt(DG, N, ec.TrialType,	ec.sub.all);
  etime{N}.otype  = selectdgevt(DG, N, ec.ObspType,	ec.sub.all);
  etime{N}.fs	  = selectdgevt(DG, N, ec.Fixspot,	ec.sub.all);
  etime{N}.stm    = selectdgevt(DG, N, ec.Stimulus,   ec.sub.all);
  etime{N}.stype  = selectdgevt(DG, N, ec.Stimtype,   ec.sub.all);
  etime{N}.cue    = selectdgevt(DG, N, ec.Cue,		ec.sub.all);
  etime{N}.tar    = selectdgevt(DG, N, ec.Target,	    ec.sub.all);
  etime{N}.distr  = selectdgevt(DG, N, ec.Distractor, ec.sub.all);
  etime{N}.sound  = selectdgevt(DG, N, ec.Sound,		ec.sub.all);
  etime{N}.fix	  = selectdgevt(DG, N, ec.Fixate,	    ec.sub.all);
  etime{N}.resp	  = selectdgevt(DG, N, ec.Response,	ec.sub.all);
  etime{N}.eot	  = selectdgevt(DG, N, ec.EndTrial,	ec.sub.all);
  etime{N}.eotcor = selectdgevt(DG, N, ec.EndTrial,	ec.sub.EndTrialCorrect);
  etime{N}.abort  = selectdgevt(DG, N, ec.Abort,		ec.sub.all);
  etime{N}.rwd	  = selectdgevt(DG, N, ec.Reward,	    ec.sub.all);
  etime{N}.dly	  = selectdgevt(DG, N, ec.Delay,		ec.sub.all);
  etime{N}.pnsh	  = selectdgevt(DG, N, ec.Punish,	    ec.sub.all);
  etime{N}.mri    = selectdgevt(DG, N, ec.Mri,	    ec.sub.MriTrigger);
  etime{N}.paton  = selectdgevt(DG, N, ec.Pattern,    ec.sub.PatternOn);
  etime{N}.patset = selectdgevt(DG, N, ec.Pattern,    ec.sub.PatternSet);
  etime{N}.patoff = selectdgevt(DG, N, ec.Pattern,    ec.sub.PatternOff);
  % new events, Jul.06
  etime{N}.injection     = selectdgevt(DG, N, ec.Injection,     ec.sub.all);
  etime{N}.posture       = selectdgevt(DG, N, ec.Posture,       ec.sub.all);
  etime{N}.bmparams      = selectdgevt(DG, N, ec.BmParams,      ec.sub.all);
  etime{N}.vdaqStimReady = selectdgevt(DG, N, ec.VdaqStimReady, ec.sub.all);
  etime{N}.vdaqStimTrig  = selectdgevt(DG, N, ec.VdaqStimTrig,  ec.sub.all);
  etime{N}.vdaqGo        = selectdgevt(DG, N, ec.VdaqGo,        ec.sub.all);
  etime{N}.vdaqFrame     = selectdgevt(DG, N, ec.VdaqFrame,     ec.sub.all);
  etime{N}.revcorrInfo   = selectdgevt(DG, N, ec.RevcorrInfo,   ec.sub.all);
  etime{N}.revcorrUpdate = selectdgevt(DG, N, ec.RevcorrUpdate, ec.sub.all);
  ...
  if any(diff(etime{N}.mri) < 0)
    ...
    etime{N}.mri = sort(etime{N}.mri);
  end
  
  
  % Apr/May-03, for new ess system: MriGeneric.c
  eparams{N}.prm     = selectdgprm(DG, N, ec.Floats_1);
  eparams{N}.stmid = [];
  eparams{N}.trialid = selectdgprm(DG, N, ec.TrialType, 1);
  eparams{N}.stmid   = selectdgprm(DG, N, ec.Stimtype,  1);
  eparams{N}.stmdur  = selectdgprm(DG, N, ec.Stimulus,  1);

  
  % new parameters, Jul.06
  eparams{N}.obsptype      = selectdgprm(DG, N, ec.ObspType,      ec.sub.all);
  eparams{N}.injection     = selectdgprm(DG, N, ec.Injection,     ec.sub.all);
  eparams{N}.posture       = selectdgprm(DG, N, ec.Posture,       ec.sub.all);
  eparams{N}.bmparams      = selectdgprm(DG, N, ec.BmParams,      ec.sub.all);
  eparams{N}.vdaqStimReady = selectdgprm(DG, N, ec.VdaqStimReady, ec.sub.all);
  eparams{N}.vdaqStimTrig  = selectdgprm(DG, N, ec.VdaqStimTrig,  ec.sub.all);
  eparams{N}.vdaqGo        = selectdgprm(DG, N, ec.VdaqGo,        ec.sub.all);
  eparams{N}.vdaqFrame     = selectdgprm(DG, N, ec.VdaqFrame,     ec.sub.all);
  eparams{N}.revcorrInfo   = selectdgprm(DG, N, ec.RevcorrInfo,   ec.sub.all);
  eparams{N}.revcorrUpdate = selectdgprm(DG, N, ec.RevcorrUpdate, ec.sub.all);
  eparams{N}.screenInfo    = selectdgprm(DG, N, ec.ScreenInfo,    ec.sub.all);

  % for awake MRI
  eparams{N}.emscale = selectdgprm(DG, N, ec.EmParams,    ec.sub.EmScale);
  ...
  if strncmpi(DG.e_pre{1}{2},'MriFixate',9) || strncmpi(DG.e_pre{1}{2},'MriPsycho',9)
    ...
    etime{N}.stm       = selectdgevt(DG, N, ec.Stimulus,   ec.sub.NKLStimulusOn);
    etime{N}.stype     = selectdgevt(DG, N, ec.Stimtype,   ec.sub.all);
    eparams{N}.stmid   = selectdgprm(DG, N, ec.Stimulus,  3, ec.sub.NKLStimulusOn);
    eparams{N}.stmdur  = selectdgprm(DG, N, ec.Stimulus,  1, ec.sub.NKLStimulusOn);
...
tmpnames     = selectdgprm(DG, 1, ec.Strings_1, 0);
ExpEvt.prmnames = {};
if ~isempty(tmpnames)
...
tmpv = selectdgprm(DG, 1, ec.Stimulus,2, 2);
...
if strcmpi(ExpEvt.system,'MriFixate') || isfield(grp,'firstMriTrigger')
  ExpEvt = sub_fix_MriFixate(ExpEvt,grp,ExpNo);
end

if strcmpi(ExpEvt.system,'MriGeneric')
  if isfield(Ses.expp(ExpNo),'ShiftedMriTrigger') && any(Ses.expp(ExpNo).ShiftedMriTrigger)
    ExpEvt = sub_fix_ShiftedMriTrigger(ExpEvt,grp,ExpNo);
  elseif isfield(grp,'ShiftedMriTrigger') && any(grp.ShiftedMriTrigger)
    ExpEvt = sub_fix_ShiftedMriTrigger(ExpEvt,grp,ExpNo);
  end
end
...
% FIX PROBLEMS,
ObspParams = selectdgprm(DG, 1, 23);  % 23 as ObspType
...
if isfield(grp,'firstMriTrigger') && any(grp.firstMriTrigger > 1)
  ...
  %  return
  %end
  fprintf(' fixing MriFixate(dummies,exp=%d)...',ExpNo);
end
...
function evt = sub_fix_ShiftedMriTrigger(evt,grp,ExpNo)
  
fprintf(' fixing ShiftedMriTrigger(exp=%d)...',ExpNo);
