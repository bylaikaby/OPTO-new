...
%    2.00 30.01.12 YM  save data to each exp's file.
%    2.01 17.07.13 YM  use sigsave() for sesversion()>=2.
%    2.02 12.05.15 YM  use pv_getpvpars().
%    2.03 26.05.15 YM  use pvpar.prescan as adfoffset when available.
%    2.04 11.10.19 YM  clean up.
%    2.05 12.11.19 YM  use expget"dg"evt instead of expgetevt.
%
%  See also SESDUMPPAR EXPGETPAR_SPIKE2 EXPFILENAME EXPDGGETEVT
%           GETPVPARS PV_GETPVPARS
%           STM_READ PDM_READ HST_READ RFP_READ SIGSAVE
...
if bSave == 0
  if sesversion(Ses) >= 2
    fname = sigfilename(Ses,ExpNo,VarName);
  else
  ...
  try
    ...
    fprintf('\n ERROR %s : ''%s'' not found in %s.',mfilename,VarName,fname);
    fprintf('\n run sesdumppar(''%s'',%d) first.\n',Ses.name,ExpNo);
    keyboard
  end
  ...
  ExpPar.adf   = subValidateAdf(grp, ExpPar.adf, ExpPar.evt, ExpPar.stm, ExpPar.pvpar);
...
if isimaging(grp)
  if isa(Ses,'csession')
    scanreco = Ses.scanreco(ExpNo);
  else
    scanreco = Ses.expp(ExpNo).scanreco;
  end
  USE_JPCODE = 0;
  if any(USE_JPCODE)
    fpath = expfilename(Ses,ExpNo,'pvdir');
    pvpar = getpvpars(fpath,scanreco(1),scanreco(2));
    if ~pvpar.imgtr
    ...
    clear acqp fpath scanreco;
  else
    if any(Ses.expp(ExpNo).scanreco(2))
      pvpar = pv_getpvpars(expfilename(Ses,ExpNo,'2dseq'));
    else
      pvpar = pv_getpvpars(expfilename(Ses,ExpNo,'fid'));
    end
  end
else
  pvpar = {};
...
if exist(expfilename(Ses,ExpNo,'evt'),'file')
  evt = expgetdgevt(Ses,ExpNo);
  ...
  if isfield(Ses.expp(ExpNo),'physfile') && ~isempty(Ses.expp(ExpNo).physfile)
    fprintf(' WARNING %s : ''%s'' not found.\n',mfilename,expfilename(Ses,ExpNo,'evt'));
  end
...
if ~isempty(evt)
  if isrecording(grp)
    [NoChan,NoObsp,sampt,obslen] = adf_info(expfilename(Ses,ExpNo,'phys'));
...
if isfield(grp,'daqver') && grp.daqver >= 2 && exist(expfilename(Ses,ExpNo,'evt'),'file')
  stmpars = stm_read(expfilename(Ses,ExpNo,'stm'));
  pdmpars = pdm_read(expfilename(Ses,ExpNo,'pdm'));
  hstpars = hst_read(expfilename(Ses,ExpNo,'hst'));
else
...
if sesversion(Ses) >= 2
  sigsave(Ses,ExpNo,VarName,ExpPar,'verbose',0);
else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function adf = subValidateAdf(grp,adf,evt,stm,pvpar)
...
if isempty(evt.validobsp)
  ...
  elseif isfield(pvpar,'prescan') && any(pvpar.prescan)
    adf.adfoffset = pvpar.prescan;
  else
    adf.adfoffset = 0;
  end
...
elseif isfield(pvpar,'prescan') && any(pvpar.prescan)
  ...
  if ~isimaging(grp)
    if any(strcmpi('alert',grp.expinfo))
      for N = NoObsp:-1:1
