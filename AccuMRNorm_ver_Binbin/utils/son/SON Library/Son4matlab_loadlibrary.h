...
// #define NOBIGFILES.
// Note that the library must be built with NOBIGFILES defined! Just changing it in the
...
typedef TAdc * TpAdc;
typedef TSTime *TpSTime;
typedef TMarker * TpMarker;
typedef TAdcMark * TpAdcMark;
typedef TRealMark * TpRealMark;
typedef TTextMark * TpTextMark;
typedef char * TpStr;
typedef const char * TpCStr;
typedef WORD * TpWORD;
typedef BOOLEAN * TpBOOL;
typedef float * TpFloat;
typedef void * TpVoid;
typedef short * TpShort;
typedef TMarkBytes * TpMarkBytes;
...
SONAPI(short) SONCreateFileEx(TpCStr name, int nChannels, WORD extra, int iBigFile);
