 ...
 *  AUTHOR
 *    DLS
 *    YM  15.Jul.2013  "long" as "int32" to support 64bit *nix system.
 *
 ************************************************************************/
...
void read_int32(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) val = flipint32(val);
...
void read_string(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) length = flipint32(length);
  if (length) {
...
void read_int32s(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) nint32s = flipint32(nint32s);
  ...
  if (nint32s) {
    ...
    if (dfFlipEvents) flipint32s(nint32s, vals);
  }

  fprintf(OutFP, "%-20s\t%d\n", dfGetTagName(type), nint32s); 
...
void read_shorts(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) nshorts = flipint32(nshorts);
...
void read_floats(char type, FILE *InFP, FILE *OutFP)
{
  ...
  if (dfFlipEvents) nfloats = flipint32(nfloats);
...
int vread_int32s(char type, int *n, FILE *OutFP)
{
  ...
  if (dfFlipEvents) nvals = flipint32(nvals);

  if (nvals) {
    if (!(vals = (int32_t *) calloc(nvals, sizeof(int32_t)))) {
      fprintf(stderr,"dfutils: error allocating space for int32 array\n");
      exit(-1);
    }
    memcpy(vals, vl, sizeof(int32_t)*nvals);

    if (dfFlipEvents) flipint32s(nvals, vals);
  }
...
int dfuFileToDataFile(FILE *InFP, DATA_FILE *df)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case D_DFINFO_TAG:
      ...
      break;
    case D_NOBSP_TAG:
      get_int32(InFP, (int32_t *) &DF_NOBSP(df));
	...
	status = DF_ABORT;
      ...
      break;
    case D_NCINFO_TAG:
      get_int32(InFP, (int32_t *) &DF_NCINFO(df));
      if (DF_NCINFO(df)) {
...
int dfuFileToDFInfo(FILE *InFP, DF_INFO *dfinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case D_FILENAME_TAG:
      ...
      break;
    case D_TIME_TAG:
      get_int32(InFP, &DF_TIME(dfinfo));
      break;
    case D_FILENUM_TAG:
      get_int32(InFP, (int32_t *) &DF_FILENUM(dfinfo));
      break;
    case D_AUXFILES_TAG:
    ...
    case D_COMMENT_TAG:
      ...
      break;
    case D_EXP_TAG:
      get_int32(InFP, (int32_t *) &DF_EXPERIMENT(dfinfo));
      break;
    case D_TMODE_TAG:
      get_int32(InFP, (int32_t *) &DF_TESTMODE(dfinfo));
      break;
    case D_NSTIMTYPES_TAG:
      get_int32(InFP, (int32_t *) &DF_NSTIMTYPES(dfinfo));
      break;
...
int dfuFileToObsInfo(FILE *InFP, OBS_INFO *oinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:
      ...
      break;
    case O_FILENUM_TAG:
      get_int32(InFP, (int32_t *) &OBS_FILENUM(oinfo));
      break;
    case O_INDEX_TAG:
      get_int32(InFP, (int32_t *) &OBS_INDEX(oinfo));
      break;
    case O_BLOCK_TAG:
      get_int32(InFP, (int32_t *) &OBS_BLOCK(oinfo));
      break;
    case O_OBSP_TAG:
      get_int32(InFP, (int32_t *) &OBS_OBSP(oinfo));
      break;
    case O_STATUS_TAG:
      get_int32(InFP, (int32_t *) &OBS_STATUS(oinfo));
      break;
    case O_DURATION_TAG:
      get_int32(InFP, (int32_t *) &OBS_DURATION(oinfo));
      break;
    case O_NTRIALS_TAG:
      get_int32(InFP, (int32_t *) &OBS_NTRIALS(oinfo));
      break;
    default:
      fprintf(stderr,"unknown event type %d\n", c);
      status = DF_ABORT;
...
int dfuFileToEvList(FILE *InFP, EV_LIST *evlist)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case E_VAL_LIST_TAG: 
      get_int32s(InFP, &EV_LIST_N(evlist), &EV_LIST_VALS(evlist));
      break;
    case E_TIME_LIST_TAG: 
      get_int32s(InFP, &EV_LIST_NTIMES(evlist), &EV_LIST_TIMES(evlist));
      break;
    default:
      fprintf(stderr,"unknown event type %d\n", c);
      status = DF_ABORT;
...
int dfuFileToEmData(FILE *InFP, EM_DATA *emdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case E_ONTIME_TAG:
      get_int32(InFP, (int32_t *) &EM_ONTIME(emdata));
      break;
    case E_RATE_TAG:
	...
	status = DF_ABORT;
      ...
      break;
    case E_PNT_DEG_TAG:
      get_int32(InFP, (int32_t *) &EM_PNT_DEG(emdata));
      break;
...
int dfuFileToSpData(FILE *InFP, SP_DATA *spdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case S_NCHANNELS_TAG:
      get_int32(InFP, (int32_t *) &SP_NCHANNELS(spdata));
...
int dfuFileToSpChData(FILE *InFP, SP_CH_DATA *spchdata)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case S_CH_CELLNUM_TAG:
      get_int32(InFP, (int32_t *) &SP_CH_CELLNUM(spchdata));
      break;
...
int dfuFileToCellInfo(FILE *InFP, CELL_INFO *cinfo)
{
  ...
  while(status == DF_OK && (c = getc(InFP)) != EOF) {
    ...
    case END_STRUCT:    
      ...
      break;
    case C_NUM_TAG:
      get_int32(InFP, (int32_t *) &CI_NUMBER(cinfo));
      break;
    case C_DISCRIM_TAG:
...
int dfuBufferToDFInfo(BUF_DATA *bdata, DF_INFO *dfinfo)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case D_EXP_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &DF_EXPERIMENT(dfinfo));
      break;
    case D_TMODE_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &DF_TESTMODE(dfinfo));
      break;
    case D_NSTIMTYPES_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &DF_NSTIMTYPES(dfinfo));
      break;
    case D_EMCOLLECT_TAG:
...
int dfuBufferToObsInfo(BUF_DATA *bdata, OBS_INFO *oinfo)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case O_FILENUM_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &OBS_FILENUM(oinfo));
      break;
    case O_INDEX_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &OBS_INDEX(oinfo));
      break;
    case O_BLOCK_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &OBS_BLOCK(oinfo));
      break;
    case O_OBSP_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &OBS_OBSP(oinfo));
      break;
    case O_STATUS_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata),
				 (int32_t *) &OBS_STATUS(oinfo));
      break;
    case O_DURATION_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &OBS_DURATION(oinfo));
      break;
    case O_NTRIALS_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &OBS_NTRIALS(oinfo));
      break;
    default:
      fprintf(stderr,"unknown event type %d\n", c);
      status = DF_ABORT;
...
int dfuBufferToEvList(BUF_DATA *bdata, EV_LIST *evlist)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case END_STRUCT:    
      ...
      break;
    case E_VAL_LIST_TAG: 
      advance_bytes += vget_int32s((int *) BD_DATA(bdata), 
...
int dfuBufferToSpChData(BUF_DATA *bdata, SP_CH_DATA *spchdata)
{
  ...
  while (status == DF_OK && !BD_EOF(bdata)) {
    ...
    case S_CH_CELLNUM_TAG:
      advance_bytes += vget_int32((int32_t *) BD_DATA(bdata), 
				 (int32_t *) &SP_CH_CELLNUM(spchdata));
      break;
    case S_CH_SOURCE_TAG:
      advance_bytes += vget_char((char *) BD_DATA(bdata), 
...
DYN_GROUP *dfuCreateDynEvData(void)
{
  ...
  for (i = 0; i < nlists; i++) 
    dfuAddDynGroupNewList(dg, "", DF_INT32, 4);
...
{
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2), val);
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2+1), time);
}
			 ...
			 int p1, int p2, int p3, int p4)
{
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2+1), time);
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2), val);
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2), p1);
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2), p2);
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2), p3);
  dfuAddDynListInt32(DYN_GROUP_LIST(evgroup,type*2), p4);
}
...
DYN_LIST *dfuCreateNamedDynList(char *name, int datatype, int increment)
{
  ...
  case DF_INT32:
    DYN_LIST_VALS(dynlist) = 
      (int32_t *)calloc(DYN_LIST_MAX(dynlist), sizeof(int32_t));
    break;
  case DF_SHORT:
    DYN_LIST_VALS(dynlist) = 
...
DYN_LIST *dfuResetDynListToType(DYN_LIST *dynlist, int datatype, int increment)
{
  ...
  case DF_INT32:
    DYN_LIST_VALS(dynlist) = 
      (int32_t *)realloc(DYN_LIST_VALS(dynlist), increment*sizeof(int32_t));
    break;
  case DF_SHORT:
    DYN_LIST_VALS(dynlist) = 
...
/***********************************************************************
 *
 * dfuAddDynListInt32(DYN_LIST *, int32_t val)
 *
...
void dfuAddDynListInt32(DYN_LIST *dynlist, int32_t val)
{
  int32_t *vals;
  if (!dynlist) return;
  ...
  if (DYN_LIST_N(dynlist) == DYN_LIST_MAX(dynlist)) {
    ...
    vals = (int32_t *) realloc(vals, sizeof(int32_t)*DYN_LIST_MAX(dynlist));
  }
...
/***********************************************************************
 *
 * dfuPrependDynListInt32(DYN_LIST *, int32_t val)
 *
...
void dfuPrependDynListInt32(DYN_LIST *dynlist, int32_t val)
{
  dfuInsertDynListInt32(dynlist, val, 0);
}

/***********************************************************************
 *
 * dfuInsertDynListInt32(DYN_LIST *, int32_t val, int pos)
 *
...
int dfuInsertDynListInt32(DYN_LIST *dynlist, int32_t val, int pos)
{
  int32_t *vals;
  int i;
 ...
 * are found in the event list, they are freed.
 *
 * NOTE:  The dynamic list must be of type DF_INT32 (int32s).
 *
 ***********************************************************************/

int dfuSetEvList(EV_LIST *evlist, DYN_LIST *vals, DYN_LIST *times)
{
  if (DYN_LIST_DATATYPE(vals) != DF_INT32 ||
      DYN_LIST_DATATYPE(times) != DF_INT32) {
