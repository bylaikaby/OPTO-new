...
TAG_INFO DFTags[] = {
  ...
  { D_NOBSP_TAG,  "NOBS_PERIODS",  DF_INT32,      TOP_LEVEL },
  { D_OBSP_TAG,   "OBS_PERIOD",    DF_STRUCTURE,  OBS_P_STRUCT },
  { D_NCINFO_TAG, "NCELL_INFOS",   DF_INT32,      TOP_LEVEL },
  { D_CINFO_TAG,  "CELL_INFO",     DF_STRUCTURE,  CELL_INFO_STRUCT },
};

TAG_INFO DFInfoTags[] = {
  { D_FILENAME_TAG,  "FILENAME",   DF_STRING, TOP_LEVEL },
  { D_TIME_TAG,      "TIME",       DF_INT32,  TOP_LEVEL },
  { D_FILENUM_TAG,   "FILENUM",    DF_INT32,  TOP_LEVEL },  
  { D_COMMENT_TAG,   "COMMENTS",   DF_STRING, TOP_LEVEL },
  { D_EXP_TAG,       "EXP_ID",     DF_INT32,  TOP_LEVEL }, 
  { D_TMODE_TAG,     "TEST_MODE",  DF_INT32,  TOP_LEVEL },
  { D_EMCOLLECT_TAG, "EM_ON",      DF_CHAR,   TOP_LEVEL },
  { D_SPCOLLECT_TAG, "SPIKE_ON",   DF_CHAR,   TOP_LEVEL },
  { D_NSTIMTYPES_TAG,"NSTIMTYPES", DF_INT32,  TOP_LEVEL },
...
TAG_INFO ObsInfoTags[] = {
  { O_BLOCK_TAG,     "BLOCK_NUM",    DF_INT32,  TOP_LEVEL },
  { O_OBSP_TAG,      "OBS_NUM",      DF_INT32,  TOP_LEVEL }, 
  { O_STATUS_TAG,    "OBS_STATUS",   DF_INT32,  TOP_LEVEL },
  { O_DURATION_TAG,  "DURATION",     DF_INT32,  TOP_LEVEL },
  { O_NTRIALS_TAG,   "NTRIALS",      DF_INT32,  TOP_LEVEL },
  { O_FILENUM_TAG,   "FILENUM",      DF_INT32,  TOP_LEVEL },
  { O_INDEX_TAG,     "OBSINDEX",     DF_INT32,  TOP_LEVEL },
};
...
TAG_INFO EvListTags[] = {
  { E_VAL_LIST_TAG,  "VALUES",  DF_INT32_ARRAY,  TOP_LEVEL },
  { E_TIME_LIST_TAG, "TIMES",   DF_INT32_ARRAY,  TOP_LEVEL },
};

TAG_INFO EMDataTags[] = {
  { E_ONTIME_TAG,    "EMSTART",   DF_INT32,            TOP_LEVEL },
  ...
  { E_PNT_DEG_TAG,   "PNT_DEG",   DF_INT32,            TOP_LEVEL },
...
TAG_INFO SPDataTags[] = {
  { S_NCHANNELS_TAG,  "NSP_CHANNELS",   DF_INT32,      TOP_LEVEL },
...
TAG_INFO SPChannelTags[] = {
  ...
  { S_CH_CELLNUM_TAG, "CELLNUM",       DF_INT32,       TOP_LEVEL },
};

TAG_INFO CellTags[] = {
  { C_NUM_TAG,       "CELLNUM",           DF_INT32,            TOP_LEVEL },
...
/* record the number of cell_infos and then the structures */
  dfRecordInt32(D_NCINFO_TAG, DF_NCINFO(df));
...
/* record the number of observation periods and then the structures */
  dfRecordInt32(D_NOBSP_TAG, DF_NOBSP(df));
...
void dfRecordDFInfo(unsigned char tag, DF_INFO *dfinfo)
{
  ...
  dfRecordInt32(D_TIME_TAG, DF_TIME(dfinfo));
  dfRecordInt32(D_FILENUM_TAG, DF_FILENUM(dfinfo));
  dfRecordString(D_COMMENT_TAG, DF_COMMENT(dfinfo));
  dfRecordInt32(D_EXP_TAG, DF_EXPERIMENT(dfinfo));
  dfRecordInt32(D_TMODE_TAG, DF_TESTMODE(dfinfo));
  dfRecordInt32(D_NSTIMTYPES_TAG, DF_NSTIMTYPES(dfinfo));
...
{
  dfBeginStruct(tag);
  dfRecordInt32(O_FILENUM_TAG, OBS_FILENUM(oinfo));
  dfRecordInt32(O_INDEX_TAG, OBS_INDEX(oinfo));
  dfRecordInt32(O_BLOCK_TAG, OBS_BLOCK(oinfo));
  dfRecordInt32(O_OBSP_TAG, OBS_OBSP(oinfo));
  dfRecordInt32(O_STATUS_TAG, OBS_STATUS(oinfo));
  dfRecordInt32(O_DURATION_TAG, OBS_DURATION(oinfo));
  dfRecordInt32(O_NTRIALS_TAG, OBS_NTRIALS(oinfo));
  dfEndStruct();
}
...
void dfRecordEvList(unsigned char tag, EV_LIST *evlist)
{
  if (EV_LIST_N(evlist)) {
    dfBeginStruct(tag);
    dfRecordInt32Array(E_VAL_LIST_TAG, 
		      EV_LIST_N(evlist), EV_LIST_VALS(evlist));
    dfRecordInt32Array(E_TIME_LIST_TAG, 
...
{
  dfBeginStruct(tag);
  dfRecordInt32(E_ONTIME_TAG, EM_ONTIME(emdata));
  ...
  dfRecordInt32(E_PNT_DEG_TAG, EM_PNT_DEG(emdata));
...
void dfRecordSpData(unsigned char tag, SP_DATA *spdata)
{
  int i;
  dfBeginStruct(tag);
  dfRecordInt32(S_NCHANNELS_TAG, SP_NCHANNELS(spdata));
...
{
  dfBeginStruct(tag);
  dfRecordInt32(S_CH_CELLNUM_TAG, SP_CH_CELLNUM(chdata));
...
{
  dfBeginStruct(tag);
  dfRecordInt32(C_NUM_TAG, CI_NUMBER(cinfo));
...
void dfRecordInt32Array(unsigned char type, int n, int32_t *a)
{
  send_event(type, (unsigned char *) &n);
...
void dfRecordInt32(unsigned char type, int32_t val)
{
  send_event(type, (unsigned char *) &val);
}
