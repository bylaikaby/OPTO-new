...
%
%  VERSION :
%    0.91 16.02.12 YM  use svds() for faster computation.
%
%  See also svd svds order_mds run_mds
...
ardot = 1/n * ones(n,1) * sum(A,1);
asdot = 1/n * sum(A,2) * ones(1,n);
adotdot = 1/n/n * sum(sum(A,1)) * ones(n,n);
...
error = [];
if nargout > 1
  error = zeros(1,min(10,size(positions,1)));
