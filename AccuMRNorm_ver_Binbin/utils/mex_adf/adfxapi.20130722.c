...
int adfx_channeloffs(int ch, ADFX_HEADER *h)
{
    ...
    if (ch < 0)  n = h->nchannels_ai + h->nchannels_di;
    else    n = ch;
...
ADFX_HEADER *adfx_readHeader(FILE *fp)
{
    ...
    fseek(fp, ADFX_HEADER_STATIC_SIZE, SEEK_SET);
    ...
    header->adc2volts = (float *)calloc(n, sizeof(float));
    status = fread(header->adc2volts, sizeof(float), n, fp);
    if (!status) goto error;
    ...
    case ADF_ADFX2013_CONV:
        ...
        fseek(fp, header->offset2dir, SEEK_SET);
...
ADFX_DIR *adfx_createDirectoryEx(FILE *fp, ADFX_HEADER *h, double logicHvolts, double logicLvolts)
{
    ...
    n = ADFX_HEADER_STATIC_SIZE + (1 + 1 + 4 + 4)*nchannels;  // char*,char*,int*,float*
    fseek(fp, n, SEEK_SET);
    // fseek(fp, h->offset2data, SEEK_SET);
    obscnt = 0;
    if (h->channels[h->obsp_chan] > 0) {
        // Obsp-TTL as Analog-Input
        logicH = (short)(logicHvolts/(double)h->adc2volts[h->obsp_chan] + 0.5);
        logicL = (short)(logicLvolts/(double)h->adc2volts[h->obsp_chan] + 0.5);
        ...
        while ((nread = (int)fread(buffer, sizeof(char), nbytes, fp)) > 0) {
            ...
            for (i = 0; i < nread; i+=bytes_per_sample) {
                ...
                if (sval > logicH && prevLevel < logicL) {
                    ...
                    prevLevel = logicH+1;
#ifdef _DEBUG
                    printf("\n %8d % 6d",startoffs[obscnt],sval);
#endif
                } else if (sval < logicL && prevLevel > logicH) {
                    // goes High to Low
                    stopoffs[obscnt] = (int64_t)i + j;  // index for the first channel
#ifdef _DEBUG
                    printf("\n %8d % 6d: %d",stopoffs[obscnt],sval,(stopoffs[obscnt]-startoffs[obscnt])/bytes_per_sample);
#endif
                    obscnt++;
                    prevLevel = logicL-1;
        ...
        while ((nread = (int)fread(buffer, sizeof(char), nbytes, fp)) > 0) {
            ...
            for (i = 0; i < nread; i+=bytes_per_sample) {
                ...
                if (cval > 0 && prevLevel == 0) {
                    ...
                    stopoffs[obscnt] = (int64_t)i + j;  // index for the first channel
#ifdef _DEBUG
                    printf("\n %10lld OFF v=%d: %lld",stopoffs[obscnt],cval,(stopoffs[obscnt]-startoffs[obscnt])/bytes_per_sample);
#endif
                    obscnt++;
                    prevLevel = 0;
                    /* Make sure there's still enough space */
...
int adfx_convertFile(ADFX_HEADER *h, ADFX_DIR *d, FILE *fp, FILE *ofp)
{
    ...
    for (iobs = 0; iobs < h->nobs; iobs++) {
        n64 = (d->stopoffs[iobs] - d->startoffs[iobs])/bytes_per_sample_raw;
        h->obscounts[iobs]  = (int32_t)n64;
        h->obsoffsets[iobs] = offset64;
        if (h->obscounts[iobs] > maxobs64) maxobs64 = h->obscounts[iobs];  /* keep track of longest obsp */
        offset64 += n64*bytes_per_sample_cnv;
    }
    ...
    h->magic[2] = magic_adfx_cnv[2];
    h->magic[3] = magic_adfx_cnv[3];
    h->offset2dir  = ADFX_HEADER_STATIC_SIZE + (1 + 1 + 4 + 4)*nChanCnv;
    h->offset2data = ADFX_HEADER_STATIC_SIZE + (1 + 1 + 4 + 4)*nChanCnv + (4 + 8)*h->nobs;
    ...
    fwrite(h->adc2volts,  sizeof(float),   nChanCnv, ofp);
        ...
        for (ich = 0; ich < nChanRaw; ich++) {
...
#ifdef _WIN32
            _fseeki64(fp, offset64, SEEK_SET);
#else
            fseek(fp, offset64, SEEK_SET);
#endif
            ...
            case 'i':
                ...
                for (ir=0; ir<nr; ir++) {
                    fread(readbuf,sizeof(char),nread_bytes,fp);
                    kr = chanoffs_raw[ich];
                    for (k=0; k<MAX_READ_BUFFER_SIZE; k++, kr+=bytes_per_sample_raw)  ibuff[k] = *((int *)&readbuf[kr]);
...
#ifdef _DEBUG
    printf("\n chanoffs[%d]=%d, obscounts=%d, obsoffsets=%d",
           channel, chanoffs,h->obscounts[obsp],h->obsoffsets[obsp]);
#endif

#ifdef _WIN32
    _fseeki64(fp, h->offset2data + h->obsoffsets[obsp] + chanoffs, SEEK_SET);
#else
    fseek(fp, h->offset2data + h->obsoffsets[obsp] + chanoffs, SEEK_SET);
#endif
...
#ifdef _WIN32
    _fseeki64(fp,h->offset2data + h->obsoffsets[obsp] + chanoffs + startindx*sizeof(short), SEEK_SET);
#else
    fseek(fp,h->offset2data + h->obsoffsets[obsp] + chanoffs + startindx*sizeof(short), SEEK_SET);
#endif

    v = (short *) calloc(nread, sizeof(short));
...
void adfx_printDirectory(ADFX_HEADER *h, ADFX_DIR *d)
{
    ...
    for (i = 0; i < d->nobs; i++) {
        n = (d->stopoffs[i] - d->startoffs[i])/bytes_per_sample;
        printf("%d %.2fms [%d]\n", i, n*ms_per_sample);
    }
}
                         ...
                         int *nsamps, void **vals, char *datatype)
{
    ...
    *datatype = adfx_getDiDataType(h,iport);
    switch (*datatype) {
    case 'c':
        v = calloc(h->obscounts[obsp], sizeof(char)); 
        fread(v, sizeof(char), h->obscounts[obsp], fp);
        break;
    case 's':
...
int adfx_mkConvInfoFile(ADFX_HEADER *h, ADFX_DIR *d, FILE *fp, FILE *ofp)
{
    ...
    h->offset2dir  = ADFX_HEADER_STATIC_SIZE + (1 + 1 + 4 + 4)*nch;
    ...
    fwrite(h->adc2volts,  sizeof(float),   nch, ofp);
                                 ...
                                 int channel, int obsp,int *nsamps, short **vals)
{
    ...
    *vals   = (short *) calloc(n, sizeof(short));
    ...
    offset = h->offset2data + d->startoffs[obsp];
#ifdef _WIN32
...
//  short *s, *tmps;
    ...
    status = adfx_getObsPeriodFromRawFile(h,d,fp,channel,obsp,&n,&tmps);
...
#ifdef _WIN32
    if (fopen_s(&fp,argv[1],"rb") != 0)  fp = NULL;
#else
    fp = fopen(argv[1], "rb");
#endif
    if (!fp) {
...
#ifdef _WIN32
            if (fopen_s(&ofp,argv[2],"wb") != 0)  ofp = NULL;
#else
            ofp = fopen(argv[2], "wb");
#endif
        }
