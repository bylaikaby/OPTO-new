  ...
  HWND hWnd, u_int nAsyncMsg)
{
  ...
  if (hLstnSock == INVALID_SOCKET)  {
    ...
    /* set the listening socket to async mode?
    else {
...
#ifdef BLOCKING_SOCKET
    // if we don't use non-blocking I/O, the first call succeeds from main, all subsequent ones must be caught
...
#ifndef OLD
/* WinsockAPI replacements added here
   int CloseConn(sock, LPSTR, INPUT_SIZE, Window);
   void WSAperror (int, LPSTR, HANDLE);
   int GetBuf(SOCKET sock, int bufsize, int optname);
*/


int CloseConn(SOCKET sock, LPSTR lpstr, int sz, HWND win) {
  int cnt;
  char closebuf[INPUT_SIZE];
  if (!lpstr) lpstr=closebuf;
  if (win) WSAAsyncSelect(sock,win,0,0); //inform our client of socket being closed; don't handle errors here
  // ask other side to FIN and wait for FD_CLOSE or other error
  shutdown(sock,SD_SEND);
  cnt=1;ioctlsocket(sock,FIONBIO,&cnt);
  readfds.fd_count=1; readfds.fd_array[0]=sock;
  // from above: timeoutn.tv_sec=0; timeoutn.tv_usec=750;
  while (select(1,&readfds,NULL,NULL,&timeoutn) && cnt++<1000) {
    if (!recv(sock,lpstr,sz,0)) /* this is probably the FD_CLOSE from the other side */ break;
  }
  return closesocket(sock);
}

void WSAperror (int error, LPSTR lpstr, HANDLE hinst) {
  /* the message goes to the HINSTANCE of the current app, i.e. GetModuleHandle(NULL) normally (see main.c);
     in open_conn this is '0' since we use hf1 for error output */
  if (hinst) {
   LPVOID lpMsgBuf;
   char *outstr;
   FormatMessage(
   ...
   outstr=lpMsgBuf;
   // *(outstr+strlen(outstr)-2)=0; // the error number goes after the message, of course
   // printf("%s: %s (%d)\n",str,lpMsgBuf,error);
   MessageBox( NULL, (LPCTSTR)lpMsgBuf, lpstr, MB_OK | MB_ICONINFORMATION );
   LocalFree( lpMsgBuf );
  }
}

int GetBuf(SOCKET sock, int bufsize, int optname) {
  return setsockopt(sock,SOL_SOCKET,optname,&bufsize,sizeof(int));
}
#endif
