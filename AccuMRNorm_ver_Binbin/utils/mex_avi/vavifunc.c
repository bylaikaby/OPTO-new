...
void FlipBGR2RGB(void *buffer, int len)
{
  ...
  register int i, sz = len*3;
  register char *buf, tmpv;
...
int OpenAVI(MOVIE_DATA *m)
{
  ...
  // Create The PGETFRAME Using Our Request Mode
	...
	if (m->pgf == NULL) {
	  ...
	  mkdir("tmp");
	  if (strncmp("tmp\\",m->filename,4)) {
	    sprintf(newname,"tmp\\%s",m->filename);
	    c=newname+4;for(;;) {if(*c==':' || *c=='/' || *c=='\\') *c='_'; if (*++c==0 || c>(newname+255)) break;}
	  }
	  // mexPrintf("hier:%s:%s\n",m->filename,newname); goto on_error;
	  if (strcmp(m->filename,newname)) {
	    if ((fl=fopen(newname,"r"))) {fclose(fl); strcpy(m->filename,newname); goto _again;}
	    c=m->filename; for(;;) {if(*c=='/') *c='\\'; if (*++c==0) break;}
	    // mexPrintf(" trying to convert \"%s\"\n to standard avi format (check output file tmp_cnv.avi), please wait ...\n",m->filename);
	    // Sleep(1000);
	    sprintf(cmdstr,"vdub /i \"l:\\matlab\\mex_avi\\joblist1\" %s %s", m->filename, newname);
	    system(cmdstr);
	    strcpy(m->filename,newname);
	    goto _again;
	  }
    error = GetLastError();
