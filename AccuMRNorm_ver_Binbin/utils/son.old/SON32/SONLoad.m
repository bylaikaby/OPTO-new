...
%
% 16.01.2007 Yusuke Murayama @MPI Tuebingen
%   use SONFindSON32DLL.m to find out 'son32.dll'
...
if (strcmp(string,'pcwin')== 0)
    ...
    if (libisloaded('son32')==0)
      if 1,
        fname = SONFindSON32DLL;
        if isempty(fname),
          error('\nERROR %s: not found ''son32.dll'', check SONFindSON32DLL.m.\n',mfilename);
          return;
        end
        loadlibrary(fname,@son32);
      else
        % OLD CODE....
        try
