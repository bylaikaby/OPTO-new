...
%    0.92 06.12.10 YM   minor bug fix of 'SliderStep'.
%    0.93 11.05.11 YM   supports 'flip' in LightBox.
%    0.94 08.01.12 YM   supports RGB data.
%    0.95 28.01.12 YM   use set(h,'CurrentAxes',..).
%    0.96 17.03.14 YM   supports zoom-in in lightbox modes.
%    0.97 26.05.15 YM   supports .nii (NIfTI-1).
%    0.98 29.07.17 YM   updates for RGB data.
%
...
switch lower(eventdata),
 case {'init'}
  ...
  set(wgts.ViewModeCmb,'Value',1);  % should be "orthogonal"
  Main_Callback(hObject,'view-mode');
  ...
  ANA.datatype = hdr.dime.datatype;
  clear hdr img;

  ANAP = getappdata(wgts.main,'ANAP');

  % converts ANA.dat into RGB
  if ~strcmpi(ANA.datatype,'rgb'),
    anaminv = 0;
    anamaxv = 0;
    anagamma = 1.8;
    if isfield(ANAP,'anz_view') && isfield(ANAP.anz_view,'anascale') && ~isempty(ANAP.anz_view.anascale),
    ...
    if anamaxv == 0,
      ...
      if anamaxv == 0,  anamaxv = 255;  end
    end
    if anaminv == 0,
      tmpv = double(min(ANA.dat(:)));
      if tmpv < 0,  anaminv = tmpv;  end
      clear tmpv;
    end
    ...
    tmpi = find(size(ANA.dat) == 3);
    if length(tmpi) == 1,
      tmpv = 1:length(size(ANA.dat));
      ANA.dat = permute(ANA.dat,[tmpv(tmpv ~= tmpi) tmpi]);  % as (x,y,z,rgb)
    else
      %ANA.dat = permute(ANA.dat,[1 2 4 3]);  % as (x,y,z,rgb)
      ANA.dat = permute(ANA.dat,[2 3 4 1]);  % as (x,y,z,rgb)
    end
    ANA.ana256 = [];
    ANA.scale  = [0 255 1.0];
  end
  set(wgts.AnaScaleEdt,'Enable','on');
  set(wgts.AnaScaleEdt,'String',sprintf('%g %g  %g',ANA.scale));
  if strcmpi(ANA.datatype,'rgb'),
    set(wgts.AnaScaleEdt,'Enable','off');
  end
 ...
 case {'browse-file'}
  fp = fileparts(get(wgts.AnzFileEdt,'String'));
  if isempty(fp) || ~exist(fp,'dir'),
    fp = pwd;
  end
  [f,d] = uigetfile( ...
	  {'*.img', 'All ANALYZE-7.5 Files (*.img)'; ...
       '*.nii', 'All NIfTI-1 Files (*.nii)'; ...
	   '*.*',   'All Files (*.*)'}, ...
	  'Pick ANALYZE-7.5/NIfTI_1 file',fp);
 ...
 case {'update-cmap'}
  ...
  set(wgts.main,'CurrentAxes',wgts.ColorbarAxs);  colormap(CMAP);
...
switch lower(eventdata),
 case {'init'}
  ...
  set(wgts.SagitalSldr,   'SliderStep',[1, 2]/max(1,nX-1));
  set(wgts.CoronalSldr,   'SliderStep',[1, 2]/max(1,nY-1));
  set(wgts.TransverseSldr,'SliderStep',[1, 2]/max(1,nZ-1));
  ...
  % now draw images
  set(wgts.main,'CurrentAxes',wgts.SagitalAxs);  cla;
  if isempty(ANA.ana256),
    tmpimg = permute(squeeze(ANA.dat(iX,:,:,:)),[2 1 3]);
  else
    tmpimg = squeeze(ANA.ana256(iX,:,:));
    tmpimg = ind2rgb(tmpimg',CMAP);
  end
  
  hSag = image(1:nY,1:nZ,tmpimg);
  set(hSag,...
  ...
  set(wgts.main,'CurrentAxes',wgts.CoronalAxs);  cla;
  if isempty(ANA.ana256),
    tmpimg = permute(squeeze(ANA.dat(:,iY,:,:)),[2 1 3]);
  else
    tmpimg = squeeze(ANA.ana256(:,iY,:));
    tmpimg = ind2rgb(tmpimg',CMAP);
  end
  hCor = image(1:nX,1:nZ,tmpimg);
  set(hCor,...
  ...
  set(wgts.main,'CurrentAxes',wgts.TransverseAxs);  cla;
  if isempty(ANA.ana256),
    tmpimg = permute(squeeze(ANA.dat(:,:,iZ,:)),[2 1 3]);
  else
    tmpimg = squeeze(ANA.ana256(:,:,iZ));
    tmpimg = ind2rgb(tmpimg',CMAP);
  end
  hTra = image(1:nX,1:nY,tmpimg);
  set(hTra,...
  ...
  MINV = ANA.scale(1);  MAXV = ANA.scale(2);
  set(wgts.main,'CurrentAxes',wgts.ColorbarAxs);  cla;
  ydat = [0:255]/255 * (MAXV - MINV) + MINV;
  ...
  % tri-plot
  set(wgts.main,'CurrentAxes',wgts.TriplotAxs);  cla;
  if isempty(ANA.ana256),
    tmpimgx = squeeze(ANA.dat(:,iX,:,:));
    tmpimgy = squeeze(ANA.dat(iY,:,:,:));
    tmpimgz = squeeze(ANA.dat(:,:,iZ,:));
    tmpimgz = permute(tmpimgz,[2 1 3]);
  else
    tmpimgx = squeeze(ANA.ana256(:,iX,:));
    tmpimgy = squeeze(ANA.ana256(iY,:,:));
    tmpimgz = squeeze(ANA.ana256(:,:,iZ))';
  end
  ...
  set(wgts.TriplotAxs,'Tag','TriplotAxs');
  set(wgts.TriplotAxs,'fontsize',8,...
 ...
 case {'dir-reverse'}
  ...
  if Xrev == 0,
    corX = 'normal';   traX = 'normal';   xdir = 'normal';
  else
    corX = 'reverse';  traX = 'reverse';  xdir = 'reverse';
  end
  if Yrev == 0,
    sagX = 'normal';   traY = 'reverse';  ydir = 'normal';
  else
    sagX = 'reverse';  traY = 'normal';   ydir = 'reverse';
  end
  if Zrev == 0,
    sagY = 'reverse';  corY = 'reverse';  zdir = 'reverse';
  else
    sagY = 'normal';   corY = 'normal';   zdir = 'normal';
  end
  set(wgts.SagitalAxs,   'xdir',sagX,'ydir',sagY);
  set(wgts.CoronalAxs,   'xdir',corX,'ydir',corY);
  set(wgts.TransverseAxs,'xdir',traX,'ydir',traY);
  set(wgts.TriplotAxs,   'xdir',xdir,'ydir',ydir,'zdir',zdir);
 ...
 case {'button-sagital'}
  ...
  elseif strcmpi(click,'open'),
    % double click
    subZoomInOrtho('sagital',wgts,ANA);
  end
 ...
 case {'button-coronal'}
  ...
  elseif strcmpi(click,'open'),
    % double click
    subZoomInOrtho('coronal',wgts,ANA);
  end
 ...
 case {'button-transverse'}
  ...
  elseif strcmpi(click,'open'),
    % double click
    subZoomInOrtho('transverse',wgts,ANA);
  end
...
switch lower(eventdata),
 ...
 case {'redraw'}
  set(wgts.main,'CurrentAxes',wgts.LightboxAxs);  cla;
  ...
  if iDimension == 1,
    ...
    INFSTR = 'Sag';
    if any(Yrev),  FLIP_X = 1;  end
    if any(Zrev),  FLIP_Y = 1;  end
  elseif iDimension == 2,
    % X-Z
    nX = size(ANA.dat,1);  nY = size(ANA.dat,3);
    INFSTR = 'Cor';
    if any(Xrev),  FLIP_X = 1;  end
    if any(Zrev),  FLIP_Y = 1;  end
  else
    % X-Y
  ...
  for N = 1:length(SLICES),
    ...
    if isempty(ANA.ana256),
      ...
      tmpimg = permute(tmpimg,[2 1 3]);  % as (y,x,rgb)
    else
      if iDimension == 1,
      ...
      tmpimg = ind2rgb(tmpimg',CMAP); % as (y,x,rgb)
    end
    if any(FLIP_X),  tmpimg = flipdim(tmpimg,2);  end
    if any(FLIP_Y),  tmpimg = flipdim(tmpimg,1);  end
    ...
    tmph = image(tmpx,tmpy,tmpimg);  hold on;
    ...
    set(tmph,...
      'ButtonDownFcn','anz_view(''LightboxView_Callback'',gcbo,''button'',guidata(gcbo))');
    
    
  end
  set(wgts.main,'CurrentAxes',wgts.LightboxAxs);
  % set(wgts.LightboxAxs,...
  %     'ButtonDownFcn','anz_view(''LightboxView_Callback'',gcbo,''button'',guidata(gcbo))');
  set(wgts.LightboxAxs,'Tag','LightboxAxs','color','black');
  set(wgts.LightboxAxs,'XTickLabel',{},'YTickLabel',{},'XTick',[],'YTick',[]);
  set(wgts.LightboxAxs,'xlim',[0 nX*NCol],'ylim',[0 nY*NRow]);
  set(wgts.LightboxAxs,'YDir','normal');
...
function subZoomInOrtho(planestr,wgts,ANA)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if iDimension == 1,
  ...
  hfig = wgts.main + 1002;
  DX = ANA.ds(2);  DY = ANA.ds(3);
  tmpstr = sprintf('SAGITAL %03d-%03d %s',SLICES(1),SLICES(end),ANA.imgfile);
  tmpxlabel = 'Y (mm)';  tmpylabel = 'Z (mm)';
  nx = size(ANA.dat,2);
  ny = size(ANA.dat,3);
elseif iDimension == 2
  ...
  hfig = wgts.main + 1001;
  DX = ANA.ds(1);  DY = ANA.ds(3);
  tmpstr = sprintf('CORONAL %03d-%03d %s',SLICES(1),SLICES(end),ANA.imgfile);
  tmpxlabel = 'X (mm)';  tmpylabel = 'Z (mm)';
  nx = size(ANA.dat,1);
  ny = size(ANA.dat,3);
else
  ...
  hfig = wgts.main + 1003;
  DX = ANA.ds(1);  DY = ANA.ds(2);
  tmpstr = sprintf('TRANSVERSE %03d-%03d %s',SLICES(1),SLICES(end),ANA.imgfile);
  tmpxlabel = 'X (mm)';  tmpylabel = 'Y (mm)';
  nx = size(ANA.dat,1);
  ny = size(ANA.dat,2);
end
