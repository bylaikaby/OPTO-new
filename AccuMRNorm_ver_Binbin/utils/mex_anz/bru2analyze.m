...
%    1.15 17.07.18 YM  supports rpPRESS (fMRS).
%
...
method = pvread_method(METHODFILE,'verbose',0);
...
if isfield(method,'PVM_VoxArrSize') && any(method.PVM_VoxArrSize)
  ...
  nz = 1;
  xres = method.PVM_VoxArrSize(1);   % [mm]
  yres = method.PVM_VoxArrSize(2);   % [mm]
  zres = method.PVM_VoxArrSize(3);   % [mm]
elseif length(reco.RECO_size) == 3,
  % likely mdeft
  nx = reco.RECO_size(1);
  ny = reco.RECO_size(2);
  nz = reco.RECO_size(3);
...
if isfield(reco,'RECO_image_type') && strcmpi(reco.RECO_image_type,'COMPLEX_IMAGE')
  fprintf('\n %s ERROR: ''%s'' is not supported.\n',mfilename,reco.RECO_image_type);
  return
end
...
if isfield(reco,'RECO_time')
fprintf(fid,'RECO_time:  %s\n',reco.RECO_time);
end
fprintf(fid,'2dseq:      %s\n',TDSEQFILE);
if isfield(reco,'RECO_image_type')
fprintf(fid,'image_type: %s\n',reco.RECO_image_type);
end
