...
regressor_rois={'D:\CM033_bids_new\sub-CM033\anat\c1csub-CM033_run-15_FLASH_BETss.nii',...
    'D:\CM033_bids_new\sub-CM033\anat\c3csub-CM033_run-15_FLASH_BETss.nii'};
% Deine Mask                                                                  
GLM_mask_file='D:\CM033_bids_new\sub-CM033\anat\large_mask.nii';                                   
print_template=cellstr('D:\CM033_bids_new\sub-CM033\norm\tempWarped.nii');
% Define the data structure for OPTO and MSTIM groups
IDs = struct();

% Adding OPTO IDs
IDs.OPTO.Pos1 = [16 17 18];



% % Adding MSTIM IDs
 IDs.MSTIM.Pos1 = [];
% IDs.MSTIM.Pos2 = 47:51;
...
conditions = ["BLANK_pre","STIM"] ; % conditions in order, choose from "BLANK_pre,"STIM"
position = 'Pos1';
...
comparison_folder = sprintf("%s_cm033",taskName);
...
if comparison_mode
    ...
    condition_combinations = condition_combinations(2); %choose the conditions for iteration
    switch taskName
        case "OPTO"
                positions = string(arrayfun(@(n) sprintf('Pos%d', n), 1:length(fieldnames(IDs.OPTO)), 'UniformOutput', false));
        case "MSTIM"
                positions = string(arrayfun(@(n) sprintf('Pos%d', n), 1:length(fieldnames(IDs.MSTIM)), 'UniformOutput', false));
        case "OPTO+MSTIM"
...
% setup_GLM_directories(position,conditions,param_matfile)
    ...
    HPF=128;
    GLM_script(conditions,subject_dirs,subject_idinfo,taskName,run_tapas,threshold,HPF);
    fwec=round(xSPM.uc(3));
    produce_corrected_montage (subject_dirs,fwec,threshold);      
end
...
function [coronal_montage,axial_montage]= GLM_script(conditions,subject_dirs,subject_idinfo,taskName,run_tapas,threshold,HPF)
    ...
    for iRun = 1:length(subject_idinfo.groupIds)
        ...
        global smoothed;
        current_scan=construct_GLM_scaninfo_033(subject_idinfo.groupIds(iRun), subject_idinfo, subject_dirs,smoothed);
...
%                     run_tapas = false;
    ...
    % Regressor files do not exist, proceed with processing steps
        ...
        matlabbatch{1}.spm.tools.physio.model.noise_rois.yes.thresholds = 0.90;
    ...
    matlabbatch{1}.spm.stats.fmri_spec.timing.RT = 6; % check the TR regularity!
...
%     
    ...
    matlabbatch{4}.spm.stats.results.export{1}.montage.background = {subject_dirs.print_template};
