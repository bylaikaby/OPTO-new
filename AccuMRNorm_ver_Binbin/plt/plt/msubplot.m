...
elseif(isempty(position))
    if (min(thisPlot) < 1)
        ...
        MYSCALE1 = 0.010;   % left
        MYSCALE2 = 0.010;   % right
        MYSCALE3 = 0.012;   % top
        MYSCALE4 = 0.052;   % bottom

        PERC_OFFSET_L = 2*MYSCALE1;
        PERC_OFFSET_R = 2*MYSCALE2;
        PERC_OFFSET_T = 2*MYSCALE3;
        PERC_OFFSET_B = 2*MYSCALE4;
		...
		def_pos(1) = def_pos(1)/3.5;
		def_pos(2) = def_pos(2)/1.5;
		def_pos(3) = def_pos(3)*1.05;
		def_pos(4) = def_pos(4)*1.00;

        col_offset = 5 *def_pos(3)*(PERC_OFFSET_L+PERC_OFFSET_R)/ ...
